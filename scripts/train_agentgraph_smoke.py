...
class LiveSmokeBackend:
    ...
    async def evaluate_final_graph(
        ...
        *,
        ...
        SkillFlow admits a rollout only after its evaluator attempt succeeds.
        Keeping this boundary separate lets the formal evaluation runner retry
        an infrastructure-failed evaluator without sampling another Director
        action or re-executing the frozen Canvas construction.  Interactive
        tasks replay accepted local environment transitions before requesting
        only the first missing Agent action.
        """
    ...
    async def collect(
        ...
        *,
        ...
        if (
            ...
            raise ConfigurationError(
                f"{semantic_protocol} requires the short neutral "
