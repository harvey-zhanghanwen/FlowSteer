...
def _workflow_problem(
    ...
    """Expose ALFWorld's required step-policy interface to Flow-Director.

    SkillFlow keeps the immutable query separate from public environment/tool
    context.  After ``FINISH``, the compiled FlowSteer AgentGraph is invoked
    once per environment step with the current observation, interaction
    history, and admissible actions.  This thin adapter does not prescribe a
    topology, role template, model, or Skill.
    """

    if _dataset_key(task) != "alfworld":
        return task.question
    section = config.get("alfworld_evaluation")
    ...
    return (
        ...
        f"{contract.strip()} Do not return a prose answer."
    )
