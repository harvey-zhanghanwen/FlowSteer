#!/usr/bin/env python3
"""Freeze a TriviaQA-only training schedule and its initial cursor.
...
Every frozen step contains one train task and exactly four rollout ordinals.
"""
...
def freeze_schedule_artifacts(
    *,
    ...
    cursor = TriviaQATrainingCursorState.fresh(schedule)
    ...
    return {
        ...
        "rollouts_per_task": TRIVIAQA_ROLLOUTS_PER_TASK,
        "schedule": str(schedule_path),
        "schedule_id": schedule.content_hash,
        "source_task_count": schedule.source_task_count,
        "step_count": len(schedule.steps),
        "training_started": False,
        "validation_task_count": TRIVIAQA_EXPECTED_VALIDATION_TASK_COUNT,
        "expected_train_task_count": TRIVIAQA_EXPECTED_TRAIN_TASK_COUNT,
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="existing aligned held-out validation JSONL",
    )
    parser.add_argument(
        "--test",
    ...
    parser.add_argument(
        "--schedule-output",
        default="artifacts/triviaqa_training/formal_schedule.json",
    )
    parser.add_argument(
        "--cursor-output",
        default="artifacts/triviaqa_training/cursor_step_000000.json",
    )
    parser.add_argument(
        "--step-count",
        type=int,
    ...
    parser.add_argument(
        "--rollouts-per-task",
        type=int,
        default=TRIVIAQA_ROLLOUTS_PER_TASK,
        help="must remain four for one same-question GRPO group",
    )
    parser.add_argument(
        "--task-positions",
