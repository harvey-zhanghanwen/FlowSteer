...
| SWE-bench (Verified reserved for final evaluation) | Repository-level software issue resolution | `swebench.harness.v1`; official Docker harness `resolved`/resolved rate | Detached repository worktree with SkillFlow list/search/view/`bash`/`str_replace_editor`/test/diff Tools plus the official Codex `apply_patch` entry point | Yes, within the bounded Coding Agent Tool loop. Fixed actions use per-action exact JSON Schema. | Tool contract ready; official evaluation blocked | The v2 surface supports default source discovery, explicit tests/docs/non-Python search, long-file AST file maps, create/replace/insert/undo, general repository commands, multi-file add/update/delete/move patches through Codex, test observations and current workspace diff. It provides AST document structure plus textual reference search, not an LSP symbol/reference engine. `ACTIVE`: none. | regular-dev provides 128 validation tasks, regular train 512, and all 500 Verified tasks remain final-only. All four required public repository mirrors are present and all 128 base commits resolve. A real task-pinned no-model canary passed `str_replace_editor → Codex apply_patch → run_tests → diff → bash → cleanup`. Completion requires `last changed edit < subsequent run_tests < subsequent changed diff < complete`. Official Docker harness preflight still fails on socket permission, so no model Coding trajectory or official `resolved` receipt exists and the metric remains unmeasurable rather than 0. |
