...
class AgentRuntime:
    ...
    def _response_output_metadata(
        ...
        """Bind one artifact to its routed public inputs and Tool lineage.

        DIRECT_REUSE + NECESSARY_ADAPTATION: this is the topology-neutral
        ``artifact_version``/``input_artifact_provenance`` boundary from the
        current TriviaQA/SkillFlow continuation runtime.  HotpotQA does not use
        the TriviaQA semantic-role protocol, so only public communication and
        Tool receipts are propagated; no role or topology is prescribed here.
        """
