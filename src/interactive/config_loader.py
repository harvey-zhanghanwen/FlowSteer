...
def validate_agent_graph_config(value: Mapping[str, Any]) -> None:
    required = {"schema_version", "data", "storage", "director", "agent_graph", "grpo", "gpu"}
    ...
    gpu = value["gpu"]
    physical = [
        int(gpu["director_training_physical"]),
        int(gpu["director_inference_physical"]),
        int(gpu["probe_skill_training_physical"]),
    ]
