...
def validate_agent_graph_config(value: Mapping[str, Any]) -> None:
    ...
    if action_profile not in known_profiles:
        raise ConfigurationError(
            ...
            "stepwise ReAct continue boundary"
        )
    if action_profile in (subgraph_actions, stepwise_subgraph_actions):
        if graph.get("max_agents_per_subgraph") != 3:
            raise ConfigurationError(
                "add_subgraph profile requires max_agents_per_subgraph=3"
            )
    if action_profile in (stepwise_legacy_actions, stepwise_subgraph_actions):
        environment_runtime = value.get("environment_runtime")
        mbppplus_runtime = value.get("mbppplus_tool_runtime")
        environment_stepwise = bool(
            isinstance(environment_runtime, Mapping)
            and environment_runtime.get("enabled") is True
            and environment_runtime.get("stepwise_director") is True
        )
        mbppplus_stepwise = bool(
            isinstance(mbppplus_runtime, Mapping)
            and mbppplus_runtime.get("enabled") is True
            and mbppplus_runtime.get("max_turns_per_director_step") == 1
        )
        if not (environment_stepwise or mbppplus_stepwise):
            raise ConfigurationError(
                "the continue action requires an enabled stepwise execution runtime"
            )
