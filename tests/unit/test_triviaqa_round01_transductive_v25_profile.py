...
V25 = ROOT / "config" / "evaluation_triviaqa_round01_transductive_qa_memory_v25.yaml"
RUNNER_PATH = ROOT / "scripts" / "evaluate_completion_benchmark_round.py"
SPEC = importlib.util.spec_from_file_location(
    "evaluate_triviaqa_round01_transductive_v25_test",
    RUNNER_PATH,
)
...
def test_v25_reuses_highest_round01_and_v23_free_canvas_boundaries() -> None:
    baseline = load_yaml(ROUND01)
    v23 = load_yaml(V23)
    config = load_yaml(V25)
...
def test_v25_declares_transductive_retrieval_and_worker_first_boundary() -> None:
    config = load_yaml(V25)
