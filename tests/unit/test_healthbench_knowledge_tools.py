...
def test_v236_keeps_graph_evaluator_sampling_and_equal_tool_condition():
    from src.interactive.config_loader import load_yaml
    from tests.unit.test_completion_benchmark_round import _MODULE
    root = Path(__file__).resolve().parents[2]
    old = load_yaml(root / "config/evaluation_healthbench_professional_optional_clinical_tools_v2_35.yaml")
    new = load_yaml(root / "config/evaluation_healthbench_professional_source_separated_retrieval_v2_36.yaml")
    graph, previous = dict(new["agent_graph"]), dict(old["agent_graph"])
    graph.pop("model_catalog_path")
    previous.pop("model_catalog_path")
    assert graph == previous
    assert new["evaluation"] == old["evaluation"]
    assert new["experiment"]["seed"] == old["experiment"]["seed"]
    assert new["grpo"]["enabled"] is False
    assert new["director"]["lora"]["enabled"] is False
    direct = new["healthbench_professional_evaluation"]
    assert direct["sample_count"] == 525
    assert direct["concurrency"] == 4
    assert direct["direct_allowed_tools"] == new["healthbench_tool_runtime"]["execution_profile_allowlist"][-1]["allowed_tools"]
    assert len(direct["direct_allowed_tools"]) == 6
    _MODULE.validate_completion_benchmark_config(new)
    catalog = load_yaml(root / new["agent_graph"]["model_catalog_path"])
    for model in catalog["models"]:
        if model["metadata"]["tool_capable"] == "true":
            assert KNOWLEDGE in model["metadata"]["tool_capability_scope"].split(",")
        assert model["metadata"]["chat_template_enable_thinking"] == "true"
