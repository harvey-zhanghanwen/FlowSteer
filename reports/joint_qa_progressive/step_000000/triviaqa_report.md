...
固定项目 validation：**32** 题。Direct 与 AgentGraph 使用同一批题、同一 Qwen3.5-9B、同一公开检索观察和同一终局 evaluator。本轮未执行训练、GRPO、反向传播、优化器更新、LoRA 发布、贝叶斯后验更新或 Skill 发布。本轮没有注入 Skill。
...
| Progressive AgentGraph | 32 | 32 | 50.00% | 61.63% |

AgentGraph − Direct：**+0.00 EM**，**+4.33 F1**。
...
- `partial_or_overlong_answer`：6
- `shared_reasoning_or_model_failure_candidate`：8
