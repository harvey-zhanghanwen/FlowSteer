# WebShop initial adapter v3 source map
...
- v2 fixes the public/private observation boundary but fails Stable Zero on one
  ...
  rounds.
- v3 keeps v2's leakage fix and adds only the generic capability/action-mask and
  upstream truncation boundaries described above. v1/v2 artifacts are retained
  under their original condition directories and are never mixed into v3.
