# ALFWorld stepwise multi-Agent v7 source map

## 1. Scope

This document records the implementation boundary for
`config/evaluation_alfworld_stepwise_multiagent_v7.yaml`.  The condition keeps
the v6 official `valid_seen` tasks, model catalog, Tool implementation,
evaluator, action budget, seed and Direct protocol.  It changes only the
AgentGraph construction boundary for ALFWorld tasks whose complexity is
explicit in the public task instruction.
...
| v7 boundary | Primary source | Classification | Retained or adapted behavior |
|---|---|---|---|
...
## 5. Evaluation invariants retained from v6
...
All v7 artifacts and reports use new versioned paths under
`artifacts/alfworld_stepwise_multiagent_v7/` and
`reports/alfworld_stepwise_multiagent_v7/`.  Partial v6 results must not be
merged into v7 metrics.

## 6. Acceptance criteria

A v7 Stable Zero or formal evaluation is reportable only if:

1. selected task IDs are the frozen v6 `valid_seen` sequence;
