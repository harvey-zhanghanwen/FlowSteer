...
| Materialization HTTP worker pool and rolling completion scheduling | SkillFlow/TriviaQA `ThreadPoolExecutor(max_workers=args.concurrency)` plus `FIRST_COMPLETED` refill loop | Direct reuse; preserves frozen-order atomic checkpoints without batch-tail stalls |
...
| Self-contained fact admission | TriviaQA `_FACT_ANAPHORIC_SUBJECT`, `_FACT_QA_WRAPPER`, and relation-bearing bare-span rejection | Direct fail-closed reuse plus necessary HotpotQA extensions for query/inquiry meta-framing and lexical-only raw-question/raw-answer copies |
