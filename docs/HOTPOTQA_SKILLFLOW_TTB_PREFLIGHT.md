...
3. 主 GRPO 路径已补齐 θ-LoRA、AdamW、cosine scheduler、Python/NumPy/PyTorch/CUDA RNG 和冻结版本元数据的完整恢复 checkpoint；尚待真实单步验收。
4. W&B 已固定绑定 `zhanghanwen6660909-dut/flowsteer-hotpotqa` 与 online 模式，并接入每步 validation 和 recoverable checkpoint Artifact；尚未创建 run，因此没有 run URL。
