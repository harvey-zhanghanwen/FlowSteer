...
| `src/interactive/tool_runtime.py::{ToolRequest,ToolResult,ToolBackend,ToolRegistration,ToolRegistry,ToolCapability,ToolReceipt}`; per-dataset tool adapters remain `NOT_IMPLEMENTED` | SkillFlow `src/skillev/runtime/tools.py::{ToolRequest,ToolResult,ToolBackend,ToolRegistration,ToolRegistry}`, `runtime/contracts.py::{StructuredAction,ActionKind,BudgetVector}`, `runtime/bounded_agent.py::{_execute_action,_reserve}`; Codex `codex-rs/core/src/tools/router.rs::{ToolCall,ToolRouter::build_tool_call,dispatch_tool_call_with_terminal_outcome}` and `core/src/tools/context.rs::{ToolInvocation,ExecCommandToolOutput,ApplyPatchToolOutput}` | Immutable resource-ID registry, normalized request/result, unknown-resource rejection, public tool observations, and synchronous `invoke` semantics are directly retained | Added `ToolCapability` metadata, asynchronous timeout, and measured `ToolReceipt`; budget settlement and concrete QA/environment/coding backends remain to be connected | SkillFlow registration lacks the required dataset/schema/side-effect/version metadata and multi-Agent async receipts; Codex is a Rust product/session stack and is reference-only | All seven datasets; `ToolCapability` and `ToolReceipt` are `NEW_PROJECT_EXTENSION` |
