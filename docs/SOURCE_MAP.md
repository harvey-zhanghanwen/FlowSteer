...
| `rollout_collector.py::SGLangReceiptDirectorClient._propose_hierarchical_action` (ADD relation-prefix selector serialization retry); `_validate_v3_hierarchical_action_receipt` | DIRECT_REUSE / necessary SGLang compatibility adaptation | Direct reuse of the existing `set_relation` selector's `_hierarchical_selector_serialization_failed` classification, one schema-bound `_PARAMETER_REGENERATION_CONTINUATION`, exact phase receipt, unchanged route/seed, and fail-closed parsing | Handle only JSON serialization/truncation in a relation-prefix selector. The malformed sample remains in `add_relation_candidate_serialization_failure_{ordinal}`; one retry uses the identical prefix schema. No default candidate, text inference, topology change, or additional retry is allowed, and receipt replay proves the prompt/seed/final relation correspondence. |
