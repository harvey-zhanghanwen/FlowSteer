...
| --- | --- |
| **Direct reuse** | FlowSteer progressive Canvas `edit -> execute -> feedback` semantics, explicit `FINISH`, action masks and trajectory records; SkillFlow `StructuredAction`, Tool registry contracts, `RetrievalIndex`, bounded computation executors, RAGEN environments, MedRAG BM25 assets, SWE-bench worktree/repository primitives and evidence-library contracts. |
| **Necessary adaptation** | Free AgentGraph validation/scheduling, task-scoped `reasoning|react|coding` dispatch, SQLite thread affinity, evaluator-locked RAGEN session/replay, native environment action grammar, required interactive environment actor at FINISH, frozen MedRAG resource binding, separated SWE worktree ownership, unified native-evaluator receipts, and exact model-capability admission. |
...
| **Not implemented/executed** | A live SWE-bench Coding trajectory and valid official-harness `resolved` receipt, evidence-gated multidataset `ACTIVE` Skills, Skill injection, and multidataset micro-training/optimizer/policy-sync execution. Six other fixed conditions produced evaluator-valid two-task Stable Zero receipts. |
...
| `src/interactive/tool_runtime.py::{StructuredAction,ToolRequest,ToolResult,ToolBackend,ToolRegistration,ToolRegistry,ToolCapability,ToolReceipt}`; `react_execution.py::ToolReactExecutionAdapter` | SkillFlow `src/skillev/runtime/tools.py::{ToolRequest,ToolResult,ToolBackend,ToolRegistration,ToolRegistry}`, `runtime/contracts.py::{StructuredAction,ActionKind,BudgetVector}`, `runtime/bounded_agent.py::{_execute_action,_reserve}` | Immutable resource-ID registry, normalized request/result, unknown-resource rejection, public Tool observations, explicit completion, and one action/observation turn are direct reuse | Necessary adaptation adds task-scoped capability metadata and async timeout. `ToolCapability` and measured `ToolReceipt` are project additions needed for heterogeneous AgentGraph replay. Concrete QA, environment, computation, MedRAG and repository backends are connected through this registry | SkillFlow registration has no dataset/schema/side-effect/version metadata or multi-Agent asynchronous receipt; the new fields do not replace its execution contract | All seven datasets |
| `src/interactive/qa_retrieval.py::{SkillFlowQARetriever,QARetrievalReceipt,augment_task_with_retrieval}`; `qa_tool_adapter.py::{QASearchToolBackend,QAReadToolBackend,open_qa_tool_registry}`; `react_execution.py::ToolReactExecutionAdapter` | SkillFlow `src/skillev/benchmarks/retrieval.py::{RetrievalIndex.open,RetrievalIndex.search,RetrievalIndex.read,QARetrievalEnvironment.execute,QARetrievalEnvironment.validate_completion}` | Read-only versioned `RetrievalIndex`, answer-free `search`/`read` schemas, public passage IDs/text, corpus/index identity and explicit Tool/complete actions are direct reuse | Necessary adaptation registers search/read as task-scoped resources, keeps the SQLite connection on one thread, and persists query, top-k, passage IDs, observations, latency and corpus version. `LiveSmokeBackend._runtime_for_task` and `evaluate_completion_benchmark_round.py` now expose frozen, protocol-separated QA conditions | The historical deterministic-prefetch wrapper remains only for legacy results; it is not used by the new model-driven Tool condition. Closed-context/question-only Direct and retrieval-enabled AgentGraph scores remain separate protocols | HotpotQA and TriviaQA |
| `src/interactive/computation_tools.py::{AIMEComputationToolBackend,create_aime_computation_registry}`; `react_execution.py::ToolReactExecutionAdapter` | SkillFlow `training/tools.py::{execute_tool,_calculator,_python_exec,_exec_code_in_process}` | Calculator namespace, Python child-process execution and hard timeout are direct reuse | Necessary adaptation exposes both functions as AIME-scoped Tool resources and omits SkillFlow's Tool success/failure reward. The two-task Stable Zero is evaluator-valid, but the Director selected zero optional computation Tool calls, so Tool use remains unvalidated | The upstream functions are coupled to SkillFlow's training environment and reward path; only their public observation-producing execution is reused | AIME 2026 |
| `src/interactive/healthbench_tool_adapter.py::{FrozenMedRAGBM25Corpus,HealthBenchMedRAGSearchToolBackend,open_healthbench_medrag_tool_registry}`; `react_execution.py::ToolReactExecutionAdapter` | SkillFlow MedRAG textbooks runtime and its formal preflight/BM25 scoring boundary | Frozen textbook corpus, source revision/row-count binding, tokenization, BM25 constants, top-3 result projection and public snippet text are direct reuse | Necessary adaptation registers one HealthBench-only read-only resource and keeps evaluator rubrics, reference answer and judge state outside Tool inputs. The two-task Stable Zero is evaluator-valid; its only MedRAG call returned `ValueError`, so successful Tool use remains unvalidated | SkillFlow's released task list does not provide a HealthBench AgentGraph Tool boundary; this adapter exposes only the existing frozen MedRAG corpus | HealthBench Professional |
| `src/interactive/environment_execution.py::{EnvironmentExecutionAdapter,RAGENEnvironmentSession,RAGENEnvironmentSessionFactory,evaluator_locked_ragen_session_factory}`; `agent_workflow_env.py::{required_tool_id,required_tool_issue}`; `task_evaluator.py::_evaluate_environment` | SkillFlow deployed `src/ragen_adapter.py::{RAGENAdapter.reset,RAGENAdapter.step,ALFWorldEnv.reset,ALFWorldEnv.step,WebShopEnv.reset,WebShopEnv.step}`; SkillFlow `runtime/bounded_agent.py::BoundedAgent.execute_turn` and `runtime/execution.py::{RolloutEnvironmentSession,EnvironmentObservation}` | Real reset, admissible native actions, environment step, public observation, bounded turns and failure observations are direct reuse | Necessary adaptation creates one request-scoped AgentRuntime resource, records typed environment revisions/Tool receipts/evaluator replay trace, and excludes reward/`won` from model-visible state. The executor's native action grammar takes precedence over free Canvas formatting, and interactive FINISH requires exactly one ReAct Agent owning the condition's environment Tool. Both changes were derived from preserved v1 failure receipts and validated on the same fixed tasks in v2 | RAGEN has no AgentGraph or typed receipt boundary. Without the thin adaptations, WebShop received JSON objects instead of native actions and ALFWorld could FINISH without a replayable environment actor | ALFWorld and WebShop |
| `src/interactive/swe_worktree.py::{SWEbenchRepositoryIdentity,PreparedSWEbenchWorktree,prepare_swebench_worktree_for_task}`; `coding_tools.py::{RepositoryToolBackend,create_swebench_repository_registry}`; `coding_execution.py::CodingExecutionAdapter`; `swebench_adapter.py::OfficialSWEbenchHarness` | SkillFlow `training/environment.py::{GenericTaskEnvironment._setup_swe_repo,cleanup,_resolve_repo_path,_handle_list_files,_handle_search_code,_handle_view_file,_handle_str_replace_editor,_generate_workspace_diff,_handle_run_tests,_run_tests_in_swe_env}` and `training/swe_bench_eval.py::evaluate_patch`; Codex tool/result loop is semantics-only reference | Base-commit detached worktree lifecycle, repository-relative list/search/view, exact edit, workspace diff, targeted test execution, Tool result returned to the next model turn, and official harness evaluation are direct reuse/thin ports | Necessary adaptation separates worktree ownership from SkillFlow's monolithic episode and connects it to one Coding Agent in the common scheduler. Two fixed base-commit worktree create/cleanup canaries passed, but the official Docker harness failed preflight; execution stopped fail-closed before any model/API/Coding trajectory, and no proxy `resolved` score is emitted | SkillFlow's repository state is single-Supervisor private state, while Codex cannot replace this Python FlowSteer runtime. The official harness remains the only source of `resolved` | SWE-bench Verified |
...
| `src/interactive/task_evaluator.py::evaluate_task`; `aime2026_adapter.py`; `swebench_adapter.py::OfficialSWEbenchHarness`; `scripts/evaluate_completion_benchmark_round.py` | HotpotQA official answer normalization; TriviaQA accepted-answer normalization; SkillFlow Protocol 10 integer evaluation and deployed RAGEN adapter; OpenAI simple-evals HealthBench rubric contract; official SWE-bench harness | Each benchmark's native answer/terminal/harness contract and primary metric are retained | Necessary adaptation normalizes evaluator receipts behind one async boundary and replays only the recorded RAGEN trajectory. Gold answers, aliases, rubrics, environment reward/`won` and SWE-bench resolution remain evaluator-only. The generic fixed-condition runner now registers all seven dataset keys | A generated answer/diff or invalid evaluator cannot be replaced with a proxy score | All seven datasets |
| `scripts/report_multidataset_stable_zero.py`; `MULTIDATASET_AGENT_ARCHITECTURE_STABLEZERO_REPORT.md`; `reports/multidataset_stablezero/*.md` | Existing FlowSteer trajectory/turn/action-mask records, SkillFlow rollout manifests, and this project's unified paired-result/evaluator receipt schemas | No new execution semantics are introduced; the renderer reads only persisted manifests, paired results, trajectories, model telemetry, Tool receipts and evidence-gate publications | **Project extension:** produces Chinese receipt-backed per-dataset and consolidated reports without model, environment, evaluator, Skill publication or training calls. Missing official evidence remains explicitly unmeasurable | Neither upstream repository includes a renderer for this project's seven heterogeneous conditions and typed receipt schema | All seven datasets |
| `scripts/probe_model_capabilities.py`; `config/model_catalog_multidataset_tool_v1.yaml` | Existing exact `/v1/models` discovery and the OpenAI-compatible Agent gateway | Exact provider model IDs and the same gateway message contract are reused | Necessary adaptation admits a model only after explicit Text, `StructuredAction`, and Coding-format canaries, with no alias substitution or silent fallback. Saved remote-model receipts back the catalog; the current local Qwen3.5-9B service canary is pending | Listing a model does not establish its ReAct/Coding compatibility. The Flow-Director remains local Qwen3.5-9B and is not substituted with an API model | Heterogeneous Executor catalog for all seven datasets |
| `src/interactive/skills/{pipeline.py,validator.py,lifecycle.py,retrieval.py,schema.py}` | SkillFlow `src/skillev/evidence/schema.py::{EvidenceRecord,may_update_posterior}`, `evidence/store.py::{EvidenceStore,SplitEvidenceStores}`, `runtime/skill_library.py::{SkillLibrary,SkillLibraryState,active_documents}`, `evolution/retriever.py::{TaskConditionedSkillRetriever.retrieve,applicability_matches,RetrievalDecision}` | Physical evidence split, immutable/versioned ACTIVE library, applicability matching, eligible evidence only, lifecycle and retrieval receipt are direct reuse | The same-prefix randomized paired AgentGraph intervention and posterior evidence gate are project algorithm additions. Compatibility binds condition/action to allowed tools, execution mode, environment/workspace protocol, model, policy, Tool and evaluator versions | For the current seven-dataset unified Runtime phase, evidence-gated `ACTIVE` Skills, Skill injection and micro-training have **not been executed**. Every Stable Zero config keeps Skills, GRPO, optimizer update and policy synchronization disabled | All seven datasets |
