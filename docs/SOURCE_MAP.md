...
## TriviaQA QA-memory v15 full-native in-database condition

This condition changes the retrieval corpus and evaluation scope without
creating a second orchestration architecture. It uses the same Qwen3.5-9B
Director, progressive Canvas, AgentGraph Runtime, worker-owned ReAct Tool calls,
explicit relations and TriviaQA evaluator as v14.4.

| Current module/boundary | Classification | Primary source retained | Minimal adaptation and exclusion |
| --- | --- | --- | --- |
| `materialize_triviaqa_full_train_qa_memory.py::{project_unique_nonheldout_train,materialize_full_train_sources}` | Necessary TriviaQA scale adaptation | Existing TriviaQA `rc.nocontext` converter and paired Q-A source fields | Native train physical rows are de-duplicated by public `question_id`; conflicting duplicate Q-A payloads fail closed. The command writes only the source projection and explicitly marks it non-Agent-facing. In the configured in-database condition all declared fixed-128 Q-A source identities are retained. |
| `generate_triviaqa_qa_memory_paraphrases.py::{verify_full_native_unique_dataset_binding,reject_exact_question_identity_shortcut,LocalQwen35Paraphraser}` | Necessary TriviaQA scale adaptation over the existing paraphrase path | Existing local-Qwen v12 lexical/phrase replacement, semantic verification, answer-slot verification and `TriviaQAQAMemoryRecord` admission | Full-native binding, source-index seeds, bounded sliding concurrency, batched atomic checkpoints and revalidation of reusable admitted rows are scale-only additions. Every admitted record remains one paired paraphrased question and relation-bearing answer statement; a complete original-question substring is rejected from both fields. The bootstrap wrapper index is not an admissible formal-result corpus. |
| `triviaqa_qa_memory.py::build_triviaqa_qa_memory_index`; `build_triviaqa_qa_memory_index.py --validation-content-indexed` | Existing normalized dense index plus explicit evaluation-scope adaptation | Existing BGE L2-normalized embedding, dot-product similarity and frozen top-k/Tool budget manifest | The overlap flag is required and the manifest records `validation_content_indexed=true` and `validation_isolation_count=0`; otherwise the original held-out boundary remains the default. Q and A are embedded together. |
| `evaluation_triviaqa_qa_memory_unified_v4_v15_full_train_transductive.yaml` | Isolated inference-only configuration | v14.4 worker Tool-first routing, Qwen3.5-9B Director, model catalog and `triviaqa.official.answer.v1` EM/F1 | The fixed128 metric is labelled `in_database_transductive`, not held-out generalization. Director Tool calls, retrieval payload injection, static prefetch, Web Search, training, Skill, MACE, GRPO and LoRA remain disabled; only a child worker owns `triviaqa.qa_memory` search/read receipts. |

## TriviaQA QA-memory v15.14 provenance and terminal-format compatibility

V15.14 retains the v15 corpus, Director, Canvas action space, child-Agent Tool
ownership, evaluator and evaluation scope.  The two changes below are bounded
compatibility adaptations at existing data-plane and terminal boundaries; they
do not add a workflow template, retrieval path, role, Tool, Skill or training
method.

| Current module/boundary | Classification | Primary source retained | Incompatibility and minimal adaptation |
| --- | --- | --- | --- |
| `qa_tool_adapter.py::_qa_memory_answer_proposition`; `agent_workflow_env.py::{_exact_source_qa_memory_canonical_answer,_reasoner_evidence_provenance_issue}` | Necessary TriviaQA paired-provenance adaptation | SkillFlow's structured `search`/`read` Action--Observation and successful Tool-receipt schema; the existing FlowSteer-derived AgentGraph relation and artifact-version routing boundary | Neither upstream defines a paired TriviaQA Q-A memory whose question paraphrase can carry entity/scope information while its answer statement carries the predicate and canonical answer. Validating the answer statement alone therefore rejects valid split-surface provenance. The adapter forms one joint provenance surface only from the paired question and answer statement in the same selected successful `read` receipt, and requires the routed memory ID plus exact source-task identity before binding the canonical answer. Its proposition schema additionally admits only the bounded lexical copular-locative form `subject + be + in/at/on + canonical answer`; it preserves that evidence-surface predicate instead of inventing a semantic relation label. Wrong-source, unrouted, unread or answer-mismatched records still fail closed, and no evaluator alias or reference receipt is consulted. |
| `openai_gateway.py::build_agent_messages` for `qa_verified_answer_lineage_v2` Formatter requests | Thin terminal-format compatibility adaptation | FlowSteer `scripts/operators.py::Format` and `scripts/prompts/prompt.py::FORMAT_PROMPT`: one completed upstream solution enters one terminal Format Operator, which performs serialization rather than reasoning | The upstream generic `FORMAT_PROMPT` asks for the shortest answer span. That instruction is destructive when one TriviaQA canonical answer contains multiple jointly requested slots, because a valid field can be omitted even after verification. The adaptation keeps the same terminal Format-Operator boundary but, only for the shared `qa_verified_answer_lineage_v2` non-Hotpot path, replaces the shortest-span instruction with character-for-character copying of the single routed `Candidate answer` into exactly one `<answer>...</answer>` wrapper. The Formatter cannot retrieve, reason, verify, canonicalize, select a replacement, inspect evaluator state or add answer content; HotpotQA and generic Format paths continue to use the directly imported upstream prompt. |
