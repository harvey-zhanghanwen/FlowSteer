...
| `agent_workflow_env.py::{allowed_action_types,model_admissible_action_types,step}`; `director.py::_canvas_observation`; `train_agentgraph_smoke.py` | FlowSteer action-set correction | FlowSteer `workflow_env.py` BUILDING-state ADD/DELETE/MODIFY/FINISH handling and the project's configured `agent_graph.actions` | Canvas remains the authority for legal state transitions and returns rejection/execution feedback after every action | The configured scalar or `add_subgraph` action profile remains the parser's full legal set. The versioned HotpotQA v13 observation additionally projects the current model-admissible subset: ADD only while capacity remains, mutation actions only when their target cardinality exists, DELETE only after the replacement-takeover gate, and FINISH only after the revision-local terminal gate passes. The gate's first measured failure stage is exposed for repair. The historical `progressive_add_subgraph_then_finish` constrained-decoding condition remains only for frozen Round8 reproducibility; GRPO token-level action masks remain a separate training receipt. |
...
## HotpotQA verified answer-slot and recovery adaptation

This condition is selected only by
`agentgraph.director.hotpotqa-semantic-recovery.v13` and
`hotpotqa_verified_answer_slot_v1`.  The neutral v10 Director and every other
dataset retain their existing prompt/runtime behavior.

| Current module | Classification | Upstream source | Reused boundary | Minimal adaptation and exclusion |
| --- | --- | --- | --- | --- |
| `director.py`, `agent_workflow_env.py` | FlowSteer reuse plus necessary HotpotQA adaptation | FlowSteer progressive Canvas `edit -> execute -> feedback`, explicit `FINISH`, revision-local execution result, strict action parser, mutation validation, and quotient-graph terminal reachability | One accepted functional subgraph/edit is executed before the next Director action; invalid semantic completion is returned as Canvas feedback. The existing Canvas action shapes are reused for unconstrained Qwen generation; v13 additionally states the already-implemented `set_relation(false,false)` edge-removal semantics so relation repair does not require deleting an Agent | The HotpotQA condition adds a machine-readable Reasoner–Verifier–Formatter terminal lineage contract and exposes `preserve -> diagnose -> repair -> augment` recovery state. It does not add a structural reward, fixed answer, Ground Truth field, or benchmark-sample rule. Actual failed Agent IDs are distinguished from blocked/pending descendants. Delete is admitted only for a diagnosed failed or terminal-unreachable node after a successful same-role/same-artifact replacement owns the artifact and downstream responsibility; semantic-role replacements must first carry a valid current Reasoner, Verifier, or Formatter artifact. Invalidated correct artifacts and receipts remain retained as previous-revision recovery state but cannot satisfy the current-revision terminal gate. Format execution-mode/tool constraints are checked transactionally before the Canvas revision is committed. |
| `qa_tool_adapter.py`, `react_execution.py`, `openai_gateway.py`, `agent_runtime.py`; `task_dataset.py::hotpotqa_question_scope` | Direct SkillFlow execution-mode and structured-output reuse plus typed request adaptation | SkillFlow bounded `StructuredAction` ReAct loop, `runtime/openai_provider.py` `response_format.json_schema`, canonical public Observation continuation, and public `search/read` Action–Observation receipts; FlowSteer state-conditioned legal-action surface; the existing `build_hotpotqa_question` rendering boundary | ReAct remains `Thought -> Action(tool) -> Observation -> Thought -> Final` execution semantics; it is not an Agent role. Under `required_evidence`, the public continuation state advances through bounded `search(limit=5) -> read₁ -> read₂ -> complete` transitions. Both reads use distinct unread opaque passage IDs from the same successful ranked result, so the second hop cannot restart an identical search loop; an already sampled candidate and valid receipts survive rejected completion or later verification feedback | When the measured state admits exactly one action, the OpenAI-compatible Executor boundary receives SkillFlow's strict StructuredAction JSON Schema. Search `limit=5`, unread passage IDs, and completion `arguments.value` are request-scoped schema constraints and cannot be confused with action-envelope fields. HotpotQA v8 obtains `question_scope` from the exact terminal `Question:` span produced by the existing data loader, then represents `answer_slot` plus each `evidence_proposition` as typed structures; `proposition_index` and `answer_field` bind `candidate_answer` to one proposition, and every claimed evidence span is checked against successful read receipts before `FINISH`. The gate permits semantic coreference (the subject need not be a verbatim substring) while Verifier remains responsible for entity binding. Generic domains retain their prior parser behavior. No query, passage, entity, answer, Ground Truth, or evaluator field is injected. |
| `openai_gateway.py`, FlowSteer `scripts/prompts/prompt.py::FORMAT_PROMPT` | Direct FlowSteer Format reuse plus necessary wire adaptation | FlowSteer's terminal Format Operator only serializes a computed solution | Reasoner owns `candidate_answer`; Verifier checks evidence, entity–attribute binding, multi-hop completeness, and unchanged scope; Format receives only the Verifier artifact | Formatter input omits the original question and copies the supported `Candidate answer` character-for-character into one `<answer>` wrapper. It cannot select, canonicalize, verify, or repair an answer. The terminal gate rejects legacy untyped Reasoner fields, requires at least two structured evidence propositions, permits retrieval receipts only on the Reasoner or an upstream Retriever routed through it, and rejects a direct Retriever-to-Verifier bypass. The structured semantic fields are runtime admission fields, not hidden reasoning supervision. |
