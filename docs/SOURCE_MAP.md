...
`hotpotqa_verified_answer_slot_v1`; its bounded retrieval/recovery receipt is
`skillflow.qa-retrieval.multihop-semantic-repair.v16`. Prompts v21 and earlier remain legacy,
...
| `agent_workflow_env.py::{recovery_state,_mandatory_repair_agent_ids,_provider_repair_model_ids,_react_repair_admission_issue,_semantic_repair_attribution,_delete_admission_issue,_preservation_admission_issue,_execution_error_feedback,finish_admissibility}`; `agent_runtime.py::_public_failure_metadata`; `director.py::{_model_catalog,_canvas_observation}` | FlowSteer/SkillFlow failure-feedback reuse plus user-design-required recovery adaptation | FlowSteer returns execution/rejection/terminal diagnostics to the next Canvas turn; SkillFlow retains nonterminal provider/Tool failure as a public observation and keeps provider identity separate from model identity | Existing successful artifacts, directed communication, Output identity, failure receipts, and explicit terminal semantics remain visible and revision-bound | HotpotQA applies `preserve -> diagnose -> repair -> augment`: preserve valid evidence, semantic answer, lineage and relations; attribute the measured failure to its Agent/relation/provider; repair the existing responsible Agent before any other edit while it remains usable. A typed provider failure, including transient service failure and permanent-configuration HTTP status, admits a model-only cross-provider repair when the frozen catalog has one, otherwise an explicit same-provider fallback. ReAct repair admits exactly one contract or completion-condition edit. If that accepted repair is followed by the same Tool-plan-exhausted diagnosis without a new Tool receipt, recovery enters `augment`, keeps the failed node, transcript, receipts, evidence and relations, and admits one Evidence Retriever/Repair unit followed by a directed ingress into the exhausted Reasoner. A successful auxiliary block clears failure state only for Agents that actually produced artifacts; deferred Agents retain their SkillFlow continuation and Tool receipts. A revision-live successful artifact cannot acquire a new predecessor unless it is the measured repair target. Disconnection, provider failure, ReAct exhaustion, Tool failure, timeout and contract failure never by themselves make a node deletable. Deletion is admissible only after explicit typed `node_unusable=true` and completed same-role/same-artifact takeover. A verified semantic lineage is protected from MODIFY/SET_OUTPUT/edge mutation, and a verified terminal revision exposes only explicit FINISH. |
...
The corresponding Runtime continuation is normally same-Agent and
phase-scoped: the same Agent and failed `single`, `draft`, or `revision` phase
receives the retained public history after a Canvas repair.  The one explicit
exception is a protocol-gated recovery augmentation: a newly added ReAct
Evidence Retriever may receive the unique repair-exhausted Reasoner's
dispatched Tool Action--Observation entries and same-Tool receipts, phase-bound
to the new singleton execution and marked by
`continuation_source_agent_id`.  Source-role completion errors are excluded;
failed Tool dispatches remain because they consumed the bounded Tool budget.
The projection survives an initial target-provider failure and model repair.
It does not create a graph message edge and carries no semantic artifact,
model transcript, hidden reasoning, Ground Truth or evaluator state.
Evidence/provenance rejection may reopen an
...
| Current module | Classification | Upstream source | Reused boundary | Minimal adaptation and exclusion |
| --- | --- | --- | --- | --- |
| `director.py::{QA_DIRECTOR_SYSTEM_PROMPT_V6,AgentGraphOrchestrator.run,_compact_historical_messages,role_conditional_qa_protocol}`; `agent_workflow_env.py::step` | Direct FlowSteer semantic reuse plus necessary QA adaptation | FlowSteer `src/interactive/workflow_builder.py::{InteractiveWorkflowBuilder.run_loop,run_loop_async,TurnRecord,Trajectory}` and `src/interactive/workflow_env.py::{InteractiveWorkflowEnv.step,_step_internal,_handle_prompt_input,_execute_workflow,_format_prompt_set_success,_check_finish_constraints}`; SkillFlow `src/skillev/rollout/context.py::CanonicalInitialContextAssembler` keeps initial action guidance compact | One admitted, fully configured executable Canvas edit or functional subgraph is executed and returned as the next observation; a raw Agent declaration is not itself an execution boundary, and `FINISH` remains an explicit action | The v6 topology-neutral QA prompt contains only the policy/environment boundary: current legal action/target domains, model/Tool selection, public feedback, semantic preservation and terminal admission. It does not embed answers, a canonical role inventory, per-role workflow instructions, workflow template, retrieval recipe, fixed Agent count, role order, topology, or structural reward. Exact role and execution-profile constraints appear only in the current `action_target_domains`. The v8 target-domain receipt carries the existing `require_format_agent` condition so constrained decoding and Env admission select the same strict or historical optional branch without altering the system prompt. Prompt receipts v5 and v6 compact stale historical Canvas observations while retaining exact sampled actions, initial task/catalog context, public failure and Tool receipts, and the full current Canvas/action domain; legacy v4 preserves its full-history replay semantics. Env validation, action masking and lossless trajectory receipts are unchanged. |
| `agent_graph.py::AgentGraph`; `agent_runtime.py::{AgentRuntime.execute,_build_plan,_execute_block,_request}` | Necessary heterogeneous AgentGraph adaptation | FlowSteer `src/interactive/workflow_env.py` and `src/interactive/workflow_builder.py` provide the progressive edit--execute--feedback control-flow boundary; neither upstream codebase implements this Runtime | The FlowSteer execution boundary and feedback semantics are retained after each admitted executable revision | Model-labelled heterogeneous nodes, quotient-DAG scheduling, reciprocal communication blocks, dirty-closure recomputation and artifact-versioned reuse are project-required adaptations. They are not attributed to FlowSteer or SkillFlow as existing AgentGraph capabilities. |
| `tool_runtime.py::{ActionKind,StructuredAction,ToolRequest,ToolResult,ToolBackend,ToolRegistration,ToolRegistry,FakeTool}`; `qa_tool_adapter.py::{_ThreadAffineRetrievalWorker,QASearchToolBackend.invoke,QAReadToolBackend.invoke,QARetrievalToolBackend.invoke,open_qa_tool_registry}`; `react_execution.py::{_parse_structured_action,ToolReactExecutionAdapter.execute,_model_visible_observations,_continuation_observations}` | Direct SkillFlow contract/retrieval reuse plus necessary bounded-execution adaptation | SkillFlow `src/skillev/runtime/contracts.py::{ActionKind,StructuredAction}`, `src/skillev/runtime/tools.py::{ToolRequest,ToolResult,ToolBackend,ToolRegistration,ToolRegistry,FakeTool}`, `src/skillev/rollout/codec.py::StructuredJsonActionCodec.parse`, `src/skillev/runtime/bounded_agent.py::{BoundedAgent.execute_turn,_validate_completion,_execute_action}`, `src/skillev/runtime/execution.py::{ActionParseResult,EnvironmentObservation,RolloutEnvironmentSession}`, `src/skillev/rollout/engine.py::RolloutEngine.run`, and `src/skillev/benchmarks/retrieval.py::{RetrievalIndex.open,search,read,QARetrievalEnvironment.execute,_outcome_for_action,validate_completion}` | The StructuredAction wire contract, Tool registry/request/result contract and actual retrieval index/search/read backend are ported directly. ReAct is the bounded execution schedule `Thought -> Action -> Observation -> Thought`: one admitted action yields one public observation, with explicit completion and bounded turns | `ToolCapability`, asynchronous invocation/timeout, JSON-Schema admission, `ToolReceipt`, the thread-affine SQLite bridge and embedding generation inside the AgentGraph request path are thin compatibility/observability adaptations. TriviaQA uses one initial search/read opportunity followed, only after evidence rejection, by bounded spelling normalization, alias expansion, entity disambiguation and query rewriting with increasing top-k. Exhaustion records the operational diagnosis `knowledge_base_coverage_failure`; it is not a corpus-level oracle claim and never admits a guessed answer. Recovery r2 propagates the existing typed `tool_plan_exhausted` receipt and preserves successful reads during entity-surface/answer-slot repair. Recovery r7 requires a new successful Reasoner read after provenance rejection. Recovery r10 keeps one compact duplicate-error observation with `repeat_count`; the lossless trajectory remains unchanged. It also gives `evidence_retriever` an answer-free completion schema and validates question scope, concise entity identity, an exact predicate surface, exact evidence span and passage ID against one matching successful read receipt. Recovery r11 keeps receipt/span/alias-lineage failures on evidence recovery but routes field-shape and lexical-field alignment to structured completion repair on that same read. Recovery r12 reuses the existing Reasoner proposition shape as the Retriever's answer-free `evidence_proposition` and validates its object against the question-only `qa_answer_type_constraint`; the Retriever still cannot own `candidate_answer`, `answer_slot`, or `final_answer`. Each execute-on-edit Canvas revision invokes one bounded ReAct call; retaining public Action--Observation history and receipts across a repaired Canvas revision is the project adaptation, not a claim that SkillFlow supplies cross-Canvas continuation. |
| `qa_tool_adapter.py::{_RequiredEvidenceState,_unified_factual_action_domain,_state_conditioned_action_domain,_state_conditioned_response_schema,_public_retrieval_continuation_state,_evidence_retriever_completion_issue,_completion_error,_tool_action_error}`; `agent_workflow_env.py::{_reasoner_evidence_provenance_issue,_verifier_read_receipt_lineage,_semantic_protocol_issue}` | Necessary factual-QA and Entity Linking adaptation | SkillFlow supplies the retrieval backend and public Tool receipts; neither SkillFlow nor FlowSteer supplies these QA policies or a standalone Entity Linking service | Every accepted entity/relation/evidence claim remains grounded in an exact successful read receipt and public evidence span | Spelling normalization, alias expansion, entity disambiguation, query rewriting, increasing top-k, the `entity identity + target relation + Tool receipt` gate, typed failure attribution, alias/coreference/relation/ordinal checks and answer-slot binding are project-required adaptations. The current Entity Linking boundary is receipt-grounded structured semantic admission and continuation, not an upstream `EntityLinker` implementation. |
| `task_dataset.py::{qa_question_scope,qa_answer_type_constraint,qa_answer_cardinality_constraint}`; `openai_gateway.py::build_agent_messages`; `agent_runtime.py::_semantic_input_deferred_components`; `agent_workflow_env.py::{_requires_complete_semantic_lineage,_semantic_edit_issue_for,_semantic_protocol_issue,_contract_obligation_issue,finish_admissibility}` | Necessary shared semantic adaptation | Existing answer-free task rendering, SkillFlow Tool receipts, and FlowSteer progressive Canvas/terminal validation | `require_format_agent=true` selects the existing complete evidence--reasoning--verification--formatting lineage; ReAct remains a per-Agent execution mode | The current v43 condition requires a receipt-valid Evidence Retriever dependency before the Reasoner is scheduled, then an artifact-versioned Reasoner-to-Verifier-to-Formatter lineage before FINISH. The Formatter is a Tool-free copy-only Output sink; a generic Output cannot bypass semantic reasoning or verification. Parallel Retrievers, evidence fan-in, repair branches, reciprocal non-Formatter communication and multi-Agent functional subgraphs remain legal, so this is a terminal/data-dependency gate rather than a prompt-fixed topology. Historical `require_format_agent=false` receipts retain the optional-capability path for exact replay. Runtime-registered `(execution_mode, allowed_tools)` profiles are exposed as correlated choices; the current Evidence Retriever profile is ReAct plus `qa-retrieval`. Question-faithful qualifiers remain legal, while question-external multi-token entity precommits and explicit candidate-answer literals are rejected before execution. Neither accepted answers nor evaluator state enter Director, Agent, query, Tool, or recovery input. |
| `agent_workflow_env.py::{AgentWorkflowEvidenceLineageSnapshot,last_valid_evidence_lineage,recovery_state,_contract_obligation_issue,_recovery_continuation_handoff,_tool_continuation_projection,_role_conditional_semantic_issue,_repair_exhausted_relation_candidates}`; `agent_runtime.py::AgentRequest`; `rollout_collector.py::{_validate_v3_hierarchical_action_receipt,_last_valid_evidence_lineage_fallback,_execution_record,_runtime_summary}`; `director.py::AgentGraphOrchestrator.run` | FlowSteer/SkillFlow feedback reuse plus project-design-required recovery adaptation | FlowSteer revision-local execution feedback, transactional action rejection, state-conditioned Canvas domains and explicit terminal state; SkillFlow nonterminal public failure observations | Valid evidence, Agent artifacts and communication edges survive later repair attempts; measured failures return to the next Director turn | Recovery follows `preserve -> diagnose -> repair/augment`. A typed mandatory provider/ReAct repair exposed as the sole live action is accepted before unrelated edits; ReAct exhaustion first admits same-Agent repair, and a complete repaired call without increased Tool receipts may open non-destructive augmentation. The Canvas action mask, target domain and authoritative admission share one state projection, but do not synthesize a fixed semantic spine: they route an already admitted ingress, repair the measured responsible node or relation, and admit an optional capability only when the live constraint exposes it. Before Output selection, a receipt-valid recovery auxiliary may be routed to the measured exhausted Reasoner even though terminal reachability is not yet measurable; after Output selection the ordinary strict reachability-reduction gate remains authoritative. Source, target and direction still come from the exact live Canvas relation domain. Revision-live successful artifacts retain predecessor identity. Contracts may express responsibilities but cannot inject a concrete answer, alias mapping or concrete Tool arguments. Rejected samples remain lossless trajectory receipts but do not become later accepted-action context. A frozen answer/Runtime/graph tuple is published only after the complete FINISH gate; a `max_rounds` fallback remains explicitly non-FINISH and GRPO-ineligible. |
| `task_evaluator.py`; `evaluate_completion_benchmark_round.py` | Direct official-metric reuse plus report-only diagnosis | Existing TriviaQA official-style normalized exact match and token F1 implementation | Accepted aliases are evaluator-only and the strict 128-task denominator is unchanged | Error reporting separates accepted-answer canonicalization mismatch, official-evaluator `partial_answer_overlap`, retrieval recall failure, operational database coverage failure, relation/answer-slot binding failure, structured-output failure and reasoning failure.  A completed explicit lineage uses its final evaluator mismatch diagnosis before historical rejected-attempt markers.  These labels do not alter EM/F1 or feed back into inference. |

The recovery row above reuses only the upstream control-flow and bounded-episode
semantics named there.  `preserve -> diagnose -> repair/augment`, cross-Canvas
revision and cross-Agent continuation, replacement takeover before deletion,
artifact-version invalidation, and the frozen last-valid evidence-lineage
fallback are project-required adaptations; neither FlowSteer nor SkillFlow
contains those implementations.  In particular, FlowSteer's retained
`last_solver_result` horizon behavior is not the local revision-consistent
answer/Runtime/graph lineage gate, and SkillFlow's terminal-versus-horizon
distinction does not provide that fallback.

Recovery r13 keeps the same source boundary.  FlowSteer's state-conditioned
action mask, authoritative Canvas admission and execute-on-edit feedback remain
the orchestration authority: a `repair_exhausted` Agent is no longer an
ordinary MODIFY target, and a replacement ingress may not depend on a failed
predecessor.  SkillFlow's StructuredAction and public Tool
Action--Observation/receipt continuation remain the only execution state
carried into a same-role Retriever replacement.  Allowing the question entity
to provide contextual scope when the answer-bearing proposition expresses a
different binary relation is a necessary project adaptation for factual QA;
the successful-read receipt, exact evidence span, passage ID and predicate
...
Recovery r16 retains the same upstream boundaries and advances the QA Tool
contract to v8.  The SkillFlow-derived bounded StructuredAction loop now admits
the same normalized query only when top-k strictly increases, rejects an exact
repeated `(query, top_k)` request, and carries one exact public repair
instruction into both the next completion input and the exhausted trace.
Answer-type mismatch takes precedence over identity-only repair.  The
FlowSteer-derived Canvas transaction executes a same-role auxiliary replacement
as an isolated functional unit and exposes its ingress only after a successful
public `read` receipt; it does not clear the original typed failure or delete
the original responsibility.

The r16 main canary exposed one narrower orchestration defect.  A successful,
receipt-grounded Evidence Retriever was terminal-unreachable while an active
`Reasoner -> Verifier -> Formatter` lineage already held the correct tc1
semantic answer.  The preserved-input gate rejected the exact one-way
Retriever-to-Reasoner edge, leaving no complete graph reachability and no
admissible `FINISH`.  Recovery r17 therefore changes only
`AgentWorkflowEnv::_allows_grounded_terminal_reachability_ingress`: one
terminal-unreachable `evidence_retriever` with a valid read receipt may become
the single new predecessor of the active Reasoner.  FlowSteer's existing
relation edit invalidates and re-executes the downstream closure, and the
previous Reasoner/Verifier/Formatter artifacts remain revision-bound in the
preservation store.  Reciprocal ingress, a different source role, a different
target role and arbitrary predecessor changes are not admitted.
...
prepare-only freezes passed.  Recovery r16 advanced to Tool v8 and passed the
same static/prepare-only gates but failed its main live canary `1/2` for the
grounded reachability defect above.  Recovery r17 passed `809` unit tests,
`144` subtests, the main `2/2` Stable Zero gate and the independent
`tc_9`/`tc_10` `2/2` gate.  All four live checks had full terminal and
evaluator receipts, zero collection failure, and reused rather than
regenerated Direct predictions.  The fixed-128 run was admitted only after
those gates; its score is authoritative only after its r17 manifest/report
completes.  Training, GRPO, policy synchronization, Skills and exploration
remain disabled.
...
and otherwise retaining a same-provider fallback.  A repaired ReAct node preserves its public Tool
receipts; a second complete bounded call without receipt progress opens
non-destructive augmentation.  These are state-conditioned recovery
domains, not new Agent roles or topology templates.

The initial two-task live canary passed one complete lineage and failed one at
`max_rounds`; it is retained as failed evidence, not an accuracy estimate.
...
## WebShop stepwise Director and StructuredAction adaptation

| Current module/boundary | Classification | Primary source retained | Minimal compatibility adaptation |
| --- | --- | --- | --- |
| `environment_execution.py::{_webshop_structured_action_schema,_parse_webshop_structured_action,EnvironmentExecutionAdapter.execute}` | Direct SkillFlow wire reuse plus task-runtime adaptation | SkillFlow `rollout/codec.py::StructuredJsonActionCodec`, `runtime/contracts.py::StructuredAction`, `runtime/bounded_agent.py::BoundedAgent.execute_turn`, and `benchmarks/webshop.py::_command_from_action` | Generate the exact five-field `StructuredAction` JSON, validate its live `search`/`click` arguments, map it deterministically to the deployed WebShop native command, and execute at most one environment transition per call. When an OpenAI-compatible provider returns every semantic field but omits only the schema constants `kind='tool'` and/or `skill_id=null`, the wire adapter fills those constants and records `format_normalized`; it never selects or rewrites `name`/`arguments`, and all other malformed objects fail closed. One task-scoped episode persists across serialized calls; reward, hidden target and evaluator `info` remain evaluator-only. |
| `agent_action_parser.py::AgentActionType.CONTINUE`; `agent_workflow_env.py::{public_environment_state,_continue_issue,step}` | `PROJECT_NECESSARY_ADAPTATION` at the FlowSteer/SkillFlow boundary | FlowSteer progressive `Canvas edit -> execute -> feedback`; SkillFlow one `Action -> Observation -> next Supervisor call` schedule | FlowSteer's six graph-edit actions cannot express “leave the graph unchanged and advance the same stateful episode once.” `continue` is therefore execution control, not an Agent role or topology edit: it preserves graph revision, reruns the environment actor and dependent closure once, and becomes inadmissible at terminal/truncation. ReAct remains `execution_mode='react'`. |
| `director.py::{STEPWISE_SCALAR_DIRECTOR_SYSTEM_PROMPT,_director_stateful_feedback_projection,AgentGraphOrchestrator._canvas_observation}`; `evaluation_webshop_stepwise_director_v7.yaml::director.history_window` | Necessary public-state projection | SkillFlow Supervisor continuation receives the original task and latest public observation; FlowSteer supplies the current Canvas, typed execution feedback and legal action domain | Every post-action Director observation repeats the original public instruction, episode/revision, last action, state-advance status, current public observation, admissible native actions and remaining budget. v7 retains two recent repair pairs and one typed execution/failure/recovery diagnosis, but does not duplicate cumulative Runtime outputs or evaluator receipts into the model input. The prompt remains topology/role neutral and contains no WebShop workflow template, reward, hidden goal or evaluator result. |
| `environment_execution.py::{_prompt_observation,EnvironmentExecutionAdapter._current_public_state}`; `evaluation_webshop_stepwise_director_v7.yaml::environment_runtime.max_observation_chars` | Necessary model-input compatibility projection | SkillFlow's bounded model-input observation and lossless environment receipt boundary | Bound the Agent- and Director-visible observation to 12,000 characters and explicitly record clipping plus the original character count. The complete observation remains unchanged in the environment and evaluator receipts, so model context control cannot change evaluator replay or reward. |
| `evaluate_completion_benchmark_round.py::_execution_environment_receipts`; `evaluation_webshop_stepwise_director_v1.yaml` through `evaluation_webshop_stepwise_director_v7.yaml` | Persistence/evaluation adaptation | SkillFlow single-episode trace, its preregistered `max_action_tokens=512`, and native WebShop Average Score/Success Rate evaluator; FlowSteer trajectory receipts | Cumulative per-Director-turn prefixes are merged by `environment_episode_id` for diagnostics, while the final longest evaluator trace remains authoritative. The 512-token action bound prevents reasoning-capable providers from exhausting a smaller 256-token cap before emitting the constrained action. The fixed v7 128-task validation panel, native evaluator and Direct arm are retained; training, GRPO, MACE, Bayesian state and Skills stay disabled. |
| `rollout_collector.py::SGLangDirectorClient._post_with_retries` | Transport diagnostic adaptation | Existing FlowSteer SGLang request/retry boundary | A bounded SGLang error response and request-shape summary are retained after an HTTP rejection so context/schema failures can be classified without replaying successful trajectories. Request payload, retry admissibility and sampled policy output are unchanged. |
| `director.py::{STEPWISE_SCALAR_DIRECTOR_SYSTEM_PROMPT_V2,director_live_action_parameter_json_schema_text,AgentGraphOrchestrator._build_messages}` | Necessary state-conditioned Director adaptation | FlowSteer live Canvas action domains and progressive `Canvas edit -> execute -> feedback`; SkillFlow Supervisor continuation over the latest public environment result | Keep the Director prompt short and topology-neutral, expose exact scalar `ADD_AGENT`/`MODIFY_AGENT` parameter domains, and compact historical full environment pages while preserving the latest public state exactly. This closes the mismatch where an admissible action type did not constrain its live parameters; it adds no fixed shopping role or workflow. |
| `environment_execution.py::{_original_task_instruction,_public_transition_summary,_public_state_feedback,_action_prompt,EnvironmentExecutionAdapter._current_public_state}` | Necessary Action--Observation feedback projection | SkillFlow `BoundedAgent`/WebShop one-Action--one-Observation loop and bounded visible history; FlowSteer typed execution feedback | After every native WebShop transition, project only the original public instruction, latest action result, current public observation, remaining action budget and measured no-progress evidence to the next Director turn. The executing Agent receives its current Director-authored free-text contract. Reward, evaluator `info` and hidden target remain excluded, and complete receipts remain lossless for evaluation. ReAct is execution control, not an Agent role. |
| `agent_workflow_env.py::{public_environment_state,_stateful_no_progress_receipt,model_admissible_action_types,action_target_domains}` | Necessary state-conditioned recovery adaptation | FlowSteer action masking at the Canvas edit boundary plus the project MD `preserve -> diagnose -> repair -> augment` recovery policy | While another environment transition remains legal, a repeated public state--action transition masks blind `CONTINUE`, `DELETE_AGENT` and premature `FINISH`; first expose an in-place free-text contract repair for the preserved environment actor, then admit ordinary Agent/relation augmentation only after persistent measured no-progress. At simulator terminal state or fixed action-budget truncation, stale no-progress history no longer masks FlowSteer's explicit `FINISH`. The environment episode and prior public evidence are retained; no WebShop-specific role topology is prescribed. |
| `evaluation_webshop_stepwise_director_v10.yaml`; `model_catalog_webshop_stepwise_v10.yaml` | Isolated evaluation configuration | The fixed v7 ordered 128-task WebShop validation panel, native Average Score/Success Rate evaluator and Direct arm | Advance only the versioned stepwise Director/action-mask condition and isolated artifact paths after v9 canary exposed a truncation/FINISH mask conflict. Freeze the local Qwen3.5-9B Executor arm because it is the only arm with successful v7 execution receipts; record that condition change explicitly. Training, GRPO, MACE, Bayesian state and Skills remain disabled. |
| `environment_execution.py::{_public_transition_summary,_public_state_feedback,_action_prompt,EnvironmentExecutionAdapter._execute_turn,_failure_metadata}`; `agent_runtime.py::_public_failure_metadata`; `agent_workflow_env.py::{public_environment_state,_failure_continuation_candidate}` | Necessary state-conditioned ReAct and failure-continuation adaptation | SkillFlow `_evolution_preflight_seed.py` requirements `plan-observation-update` and `tool-observe-before-next`, its one-Action--one-Observation boundary and public Tool failure evidence; FlowSteer typed Runtime failure feedback and Canvas recovery | Retain a bounded, most-recent set of Actions attempted from the current public state; diagnose an action cycle only when the complete state--action--next-state pair repeats. A failed environment Tool call consumes one action turn, records the attempted Action and unchanged public observation without reward/`info`, and carries that current state through the existing failure continuation so the next Director decision still sees the goal, episode and budget. This uses generic ReAct execution semantics only: no Skill is activated and no shopping role, query or topology is prescribed. |
| `evaluation_webshop_stepwise_director_v11.yaml`; `model_catalog_webshop_stepwise_v11.yaml` | Isolated evaluation configuration | The frozen v10 samples, native WebShop evaluator, Direct arm, action budget, seed, concurrency and local Qwen3.5-9B catalog | Advance only the runtime/policy/versioned output coordinates for the state-conditioned ReAct and failed-action feedback condition. Training, GRPO, MACE, Bayesian state and Skills remain disabled. |
| `agent_workflow_env.py::{_uses_atomic_stateful_execution_profile,_required_tool_profile_admission_issue,_required_tool_capability_repair_domains}`; `director.py::{director_modify_agent_field_selector_json_schema_text,director_live_action_parameter_json_schema_text,director_validate_live_action_target_domains}`; `rollout_collector.py::_validate_v3_hierarchical_action_receipt` | Necessary atomic Canvas-admission adaptation | FlowSteer `workflow_env.py::{step,_step_internal}` transactional edit then `execute_each_step`; SkillFlow `src/skillev/runtime/bounded_agent.py::BoundedAgent.execute_turn` binds one parsed environment Action to one measured Tool call and public Observation | On WebShop only, creation or repair of the stateful environment actor commits the registered pair `execution_mode='react'` plus exactly one task-scoped `allowed_tools` resource in one ADD/MODIFY action. The live JSON Schema and hierarchical receipt retain the same pair; a half-configured actor is rejected before Canvas mutation or execution. Ordinary QA/coding scalar MODIFY and ALFWorld behavior are unchanged. |
| `environment_execution.py::{_webshop_parse_product_options,_webshop_clicked_option_assignments,_webshop_model_visible_actions,_public_transition_summary}`; `EnvironmentExecutionAdapter::{_current_public_state,_execute_turn}` | Direct SkillFlow visible-state reuse plus original-environment compatibility | SkillFlow `training/environment.py::{_webshop_norm,_webshop_tokens,_webshop_parse_product_options,_webshop_clicked_values,_build_webshop_visible_state_block}`; original WebShop `web_agent_site/envs/web_agent_text_env.py::item_page` assignment `session["options"][clickable_key] = clickable_name`; SkillFlow `src/skillev/runtime/bounded_agent.py::BoundedAgent.execute_turn` one-Action--one-Observation boundary | Parse only the current public product page and completed public Action receipts. Reflect original WebShop replacement semantics by retaining the latest value per option group; remove only that already-selected value from the next model-visible StructuredAction click enum. Other option values, Buy/Back/navigation, ASIN and native search remain available. Native `admissible_actions`/evaluator `legal_actions` stay lossless. A legal repeated search is public history rather than a no-progress fault; the exact public `Total results: 0` page yields only a `zero_results` outcome. Reward, hidden target and evaluator state remain excluded. |
| `evaluation_webshop_stepwise_director_v12.yaml`; `model_catalog_webshop_stepwise_v12.yaml` | Isolated prepare-only evaluation configuration | The frozen v11 ordered 128-task validation split, seed, action budget, Direct reuse, evaluator and local Qwen3.5-9B catalog | Advance only condition/policy/tool/storage/report coordinates for the atomic execution-profile and visible option-state condition. No model/API evaluation is run while preparing this profile; training, GRPO, MACE, Bayesian state and Skills remain disabled. |
| `agent_workflow_env.py::{_webshop_stateful_reciprocal_relation,_all_model_admissible_relation_candidates,step}` | Necessary live action-domain/runtime compatibility repair | FlowSteer revision-local typed `SET_RELATION` domain and two-Agent reciprocal draft/revision block; SkillFlow WebShop's single mutable episode with serialized Action--Observation transitions; existing `AgentRuntime._validate_stateful_resource_ownership` fail-closed boundary | Remove only a bidirectional relation candidate incident to the Agent that holds `webshop.environment`, and reject the same raw/manual Canvas edit before mutation. Independent and one-way relations, free Agent contracts/models/counts and every non-WebShop topology remain unchanged; Runtime retains its authoritative stateful-resource check. |
| `evaluation_webshop_stepwise_director_v13.yaml`; `model_catalog_webshop_stepwise_v13.yaml` | Isolated prepare-only evaluation configuration | The v12 fixed ordered 128-task validation split, seed, action budget, Direct reuse, native evaluator, concurrency and local Qwen3.5-9B catalog | Advance only condition/policy/tool/storage/report coordinates for the stateful-safe relation-domain condition. No model/API evaluation, training, GRPO, MACE, Bayesian update or Skill evolution is part of profile preparation. |
| `environment_execution.py::{_webshop_price_limit,_webshop_product_price,_webshop_option_targets,_webshop_purchase_preconditions,_webshop_zero_result_queries,_webshop_action_precondition_failure,_webshop_model_visible_actions,EnvironmentExecutionAdapter._execute_turn}` | SkillFlow public-state reuse plus `PROJECT_NECESSARY_ADAPTATION` for user-required action preconditions | SkillFlow `training/environment.py::{_webshop_price_limit,_webshop_product_price,_webshop_task_attr_value}`; SkillFlow `BoundedAgent` one-Action--one-Observation execution; original WebShop `engine/goal.py::get_reward` price comparator `price <= price_upper`; FlowSteer typed action mask and execution feedback | Freeze the original task instruction for the lifetime of one episode; publish exact zero-result queries, current public price, explicit task-to-option bindings and purchase blockers through the existing public progress receipt. Reject an exact known-zero query repeat and a repeated no-change click before Tool dispatch. Remove `Buy Now` only when public price evidence violates/is missing for an explicit limit, or a syntactically explicit task option binding is missing/mismatched. Incidental value overlap such as `small end table` versus `Size=Small` never becomes a hard binding. Native legal actions/evaluator replay remain lossless; hidden goal, reward and evaluator state are never consumed. SkillFlow's exact-option guard is disabled upstream, so this hard model-visible precondition is explicitly a project adaptation required by the user rather than direct upstream reuse. |
| `evaluation_webshop_stepwise_director_v14.yaml`; `test_webshop_stateful_action_policy_v14.py` | Isolated evaluation condition and directed regression coverage | The v13 ordered 128-task validation split, seed, action budget, Direct reuse, native evaluator, concurrency, Director prompt and local Qwen3.5-9B catalog | Advance only the condition/tool/policy/storage coordinates for state-dependent action preconditions. Training, GRPO, MACE, Bayesian updates, Skills, fixed roles and fixed topology remain disabled. |
| `environment_execution.py::{_webshop_product_price,_webshop_clicked_option_assignments,_webshop_option_targets,_public_transition_summary,_public_state_feedback}` | `DIRECT_REUSE` or thin adaptation of SkillFlow public WebShop state | SkillFlow `training/environment.py::{_webshop_price_limit,_webshop_product_price,_webshop_clicked_values,_webshop_option_suggestion,_build_webshop_visible_state_block}` and its `_react_history` boundary | Preserve SkillFlow's public price parsing, latest clicked option value, visible option suggestion and bounded visible task/page/Action history semantics. The thin adaptation reads the same facts from typed stepwise environment receipts rather than SkillFlow's in-process `_react_history`; it does not read the hidden goal, reward, evaluator `info` or candidate ranking. |
| `agent_workflow_env.py::step`; `EnvironmentExecutionAdapter._execute_turn`; `director.py::AgentGraphOrchestrator._canvas_observation` | `DIRECT_REUSE` of the FlowSteer progressive Canvas/runtime boundary | FlowSteer `workflow_env.py::{step,_step_internal,execute_each_step}` and its accepted Action -> execution result -> next Director observation loop | Keep one Canvas Action followed by its execution feedback before the next Director decision. WebShop native `search`/`click` remains an Agent Tool action inside that execution; no fixed Agent role, shopping workflow or topology is introduced. |
| `environment_execution.py::{_webshop_current_product_evidence,_webshop_candidate_history,_webshop_search_queries,_webshop_option_targets,_webshop_model_visible_actions,_webshop_purchase_preconditions,_webshop_action_precondition_failure,_public_transition_summary,_public_state_feedback}` | `PROJECT_NECESSARY_ADAPTATION` for stepwise state continuity and action admission | SkillFlow's public visible-state/history and option-suggestion boundaries; original WebShop native option replacement semantics; FlowSteer state-conditioned action mask and typed execution feedback | The stepwise Director re-orchestrates after every environment Action, while upstream SkillFlow's exact WebShop option guard is disabled. Therefore retain public candidate title/price/options across `Description`/`Features`/`Reviews`; reject an already executed non-zero search query as well as a known-zero query; deterministically canonicalize public numeric, unit, pack, dimension and color forms only against currently visible option values; mask conflicting alternatives in the same option group after a high-confidence requested value is selected; and publish both remaining action budget and the minimum public Actions required to purchase the current candidate. These adaptations alter only the model-visible action domain and public feedback. Native legal actions and evaluator replay remain lossless, and no hidden goal, reward or evaluator field is consumed. |
| `evaluation_webshop_stepwise_director_v15.yaml`; `test_webshop_stateful_action_policy_v15.py` | Isolated evaluation condition and directed regression coverage | The v14 ordered 128-task validation split, seed, action budget, Direct reuse, native evaluator, concurrency, Director prompt and local Qwen3.5-9B catalog | Advance only the condition/tool/policy/storage coordinates for the v15 public-state and action-admission adaptation. Training, GRPO, MACE, Bayesian updates, Skill activation/evolution, fixed WebShop roles and fixed workflow/topology remain disabled. |
| `task_evaluator.py::_evaluate_environment`; `evaluation_webshop_stepwise_director_v16.yaml`; `test_task_evaluator.py` typed-precondition replay cases | Necessary evaluator-integration adaptation | SkillFlow WebShop keeps the environment transition and reward authoritative; FlowSteer keeps typed Runtime receipts and deterministic replay before continuation | Treat a Runtime-rejected public action precondition as a typed non-state transition only when its action, observation, next observation, reward, terminal flag, reason, feedback and info agree exactly. Do not call `environment.step` for that receipt; resume replay from the unchanged public state and use the native WebShop environment for every subsequent action and terminal reward. Any altered field fails closed. v16 preserves the v15 fixed ordered 128-task split, seed, model, action budget, Direct arm and orchestration policy in a separate artifact directory so the interrupted pre-fix v15 batch cannot contaminate the official result. |

WebShop v15 is therefore an inference/evaluation-only environment adaptation.
Its Director and Agents receive only the original public instruction, public
Action--Observation receipts and derived public state.  Ground-truth product
targets, reward, `won`, evaluator state and evaluator receipts are never model
inputs.  The unified free-AgentGraph search space is unchanged, no WebShop
workflow is prescribed, and neither training nor Skill execution/evolution is
enabled.
...
## WebShop stepwise Director v17 option-dimension and candidate recovery

The v17 source priority remains SkillFlow, then FlowSteer, followed only by the
stateful WebShop compatibility layer. It changes neither the unified
AgentGraph search space nor the native WebShop evaluator.

| Current boundary | Classification | Upstream implementation retained | Minimal compatibility adaptation |
| --- | --- | --- | --- |
| `environment_execution.py::{_webshop_measurement_signatures,_webshop_required_option_constraints,_webshop_model_visible_actions,_webshop_action_precondition_failure}` | `PROJECT_NECESSARY_ADAPTATION` over SkillFlow public option matching | SkillFlow `training/environment.py::{_webshop_parse_product_options,_webshop_option_suggestion,_webshop_missing_visible_exact_attrs}`; original WebShop `web_agent_text_env.py` flattened visible option values and same-group replacement semantics | Separate an instruction requirement from a currently matching visible value. Canonicalize only direct unit aliases and pack/count word order; do not perform cross-unit conversion. Keep a requested dimension when its value is absent, remove explicit same-dimension conflicts only from the model-visible domain, and return a typed non-dispatched failure if a model emits one through the native domain. Native actions and evaluator input remain lossless. |
| `environment_execution.py::{_webshop_current_product_evidence,_webshop_candidate_history,_webshop_phrase_evidence_status,_public_transition_summary,_public_state_feedback}` | Thin adaptation of SkillFlow public state plus bounded model-input projection | SkillFlow `training/environment.py::{_webshop_visible_requirement_phrases,_build_webshop_visible_state_block}` and its public ReAct history; SkillFlow one-Action--one-Observation continuation | Preserve bounded Description/Features/Reviews text for the current ASIN and expose exact-phrase `supported`, `contradicted`, `review_only` or `unverified` status. Product title is public evidence. The status is advisory because native WebShop pages can omit official attributes; it never hard-blocks purchase, reads reward, or becomes evaluator input. Evidence is isolated by ASIN. |
| `environment_execution.py::{_webshop_repeated_search_requires_reformulation,_webshop_model_visible_actions}` | Necessary stateful recovery adaptation | SkillFlow `_append_webshop_neutral_env_feedback` records same-query reuse after `Back`; FlowSteer progressive Canvas executes each accepted edit and returns execution feedback before the next Director action | Treat a same normalized query with no intervening candidate visit as no progress, but allow the same query after an ASIN visit because original WebShop requires it to restore the result list. Once restored, already opened ASINs are removed from the model-visible domain only while an unseen candidate or next page remains. No query, candidate ranking, role, Agent count or topology is prescribed. |
| `evaluation_webshop_stepwise_director_v17.yaml`; `test_webshop_constraint_recovery_v17.py` | Isolated inference/evaluation configuration and directed regression coverage | The v16 ordered 128 validation tasks, seed, local Qwen3.5-9B, Direct reuse, action budget, concurrency, Director prompt, model catalog and official evaluator | Advance only the versioned Tool/policy/storage/report coordinates. Training, GRPO, LoRA, MACE, Bayesian updates and Skill execution/evolution remain disabled. |

## WebShop stepwise Director v18 functional-subgraph feedback

| Current boundary | Classification | Upstream implementation retained | Minimal compatibility adaptation |
| --- | --- | --- | --- |
| `agent_workflow_env.py::{model_admissible_action_types,action_target_domains,_apply_mutation}`; `config_loader.py::validate_agent_graph_config` | `DIRECT_REUSE` of FlowSteer functional-subgraph editing plus a configuration compatibility repair | FlowSteer `ADD`/`ADD_SUBGRAPH` as one Canvas edit containing one or more Operators and relations, followed by one execution and feedback boundary | Preserve the required first scalar `ADD_AGENT` edit that creates the single WebShop environment owner, then expose the existing one-to-three-Agent `ADD_SUBGRAPH` transaction for later functional units. Persistent public no-progress admits `ADD_SUBGRAPH` rather than another stateful owner. Agent count, free-text contracts, model choice, relation direction and Output remain Director-selected; no fixed shopping role or topology is introduced. |
| `director.py::{STEPWISE_COMPONENT_DIRECTOR_SYSTEM_PROMPT,_stateful_execution_result_projection,_director_stateful_feedback_projection}` | Necessary bounded feedback projection | FlowSteer next-Canvas observation contains the just-executed workflow result; SkillFlow/WebShop returns one public `Action -> Observation` before the next Supervisor decision | Preserve bounded `agent_artifacts` and `output_inbox` routing receipts alongside the original task, latest public Action--Observation, current public state and remaining budget. Full model output, cumulative Tool trace, reward, hidden target and evaluator data remain trajectory/evaluator-only. ReAct remains an Agent execution mode, not a role. |
| `agent_workflow_env.py::_stateful_candidate_admission_issue`; `agent_runtime.py::_validate_stateful_resource_ownership` | Necessary pre-commit compatibility validation | SkillFlow serializes one WebShop session through one bounded Agent; FlowSteer validates a candidate Canvas before committing an edit | Apply the Runtime's single-owner and non-reciprocal-owner invariants to the candidate graph before commit. `ADD_SUBGRAPH`, `MODIFY_AGENT` and relation edits that would create zero/two environment owners or place the owner in a reciprocal block are rejected without changing graph revision or environment state. Stateless Agents may still use independent, one-way, fan-in/fan-out, or bounded reciprocal relations that exclude the environment owner. |
| `evaluation_webshop_stepwise_director_v18.yaml`; `test_webshop_multi_agent_subgraph.py` | Inference-only configuration and directed regression coverage | v17 fixed validation panel, native evaluator, local Qwen3.5-9B catalog, action budget and stateful Action--Observation adapter | Enable the existing generic role-free `ADD_SUBGRAPH` schema after the atomic owner boundary and verify actual artifact delivery to the environment owner. Training, GRPO, LoRA, MACE, Bayesian updates and Skill execution/evolution remain disabled; no v18 Average Score or Success Rate is claimed without a separate model-backed evaluation. |
