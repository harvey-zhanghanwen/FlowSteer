...
| `agent_workflow_env.py::{allowed_action_types,step}`; `director.py::_canvas_observation`; `train_agentgraph_smoke.py` | FlowSteer action-set correction | FlowSteer `workflow_env.py` BUILDING-state ADD/DELETE/MODIFY/FINISH handling and the project's configured `agent_graph.actions` | Canvas remains the authority for legal state transitions and returns rejection/execution feedback after every action | The configured scalar or `add_subgraph` action profile is now carried into the Canvas, exposed as `admissible_action_types`, and enforced after strict parsing. New evaluation conditions use the full configured edit set; the historical `progressive_add_subgraph_then_finish` constrained-decoding condition is retained only for frozen Round8 reproducibility and is not the current architecture because it masks repair actions after feedback. GRPO token-level action masks remain a separate training receipt. |
...
`agentgraph.director.hotpotqa-semantic-recovery.v11` and
...
| Current module | Classification | Upstream source | Reused boundary | Minimal adaptation and exclusion |
| --- | --- | --- | --- | --- |
| `director.py`, `agent_workflow_env.py` | FlowSteer reuse plus necessary HotpotQA adaptation | FlowSteer progressive Canvas `edit -> execute -> feedback`, explicit `FINISH`, revision-local execution result, strict action parser, mutation validation, and quotient-graph terminal reachability | One accepted functional subgraph/edit is executed before the next Director action; invalid semantic completion is returned as Canvas feedback. The v8 Canvas action shapes are reused for unconstrained Qwen generation because deployed SGLang 0.5.15 merges mutually exclusive fields in its flat multi-action schema | The HotpotQA condition adds a machine-readable Reasoner–Verifier–Formatter terminal lineage contract and exposes `preserve -> diagnose -> repair -> augment` recovery state. It does not add a structural reward, fixed answer, Ground Truth field, or benchmark-sample rule. Delete is admitted only for a diagnosed failed or terminal-unreachable node after a successful same-role/same-artifact replacement owns the artifact, downstream responsibility, and active Output lineage; successful reachable artifacts remain protected. Format execution-mode/tool constraints are checked transactionally before the Canvas revision is committed. |
| `qa_tool_adapter.py`, `react_execution.py`, `agent_runtime.py` | Direct SkillFlow execution-mode reuse plus typed request adaptation | SkillFlow bounded `StructuredAction` ReAct loop, canonical public Observation continuation, and public `search/read` Action–Observation receipts; FlowSteer state-conditioned legal-action surface | ReAct remains `Thought -> Action(tool) -> Observation -> Thought -> Final` execution semantics; it is not an Agent role. Under `required_evidence`, the public continuation state and model-visible action domain advance through `search only -> read(exact returned passage_id) only -> complete only`, preserve an already sampled semantic candidate after premature completion is rejected, and retain the runtime gates as authoritative admission checks | The model-visible wire separates exact action-envelope constants from the argument JSON Schema, lists only admitted resource IDs, and exposes the terminal Reasoner artifact contract only in the completion-admitted state. A malformed sampled action remains complete in `react_trace`, while its next-turn canonical invalid Observation contains status/error diagnostics rather than replaying that action as an imitation target. This removes the contradictory always-visible completion/schema surface without choosing a query, passage, entity, or answer. The request-scoped semantic protocol is propagated through Runtime so HotpotQA Reasoner/Verifier guidance cannot affect AIME, TriviaQA, or legacy HotpotQA conditions. A successful non-empty `qa-retrieval.read` receipt is required before HotpotQA semantic `FINISH`; evaluator answers are unavailable to the Tool and Verifier. |
| `openai_gateway.py`, FlowSteer `scripts/prompts/prompt.py::FORMAT_PROMPT` | Direct FlowSteer Format reuse plus necessary wire adaptation | FlowSteer's terminal Format Operator only serializes a computed solution | Reasoner owns `candidate_answer`; Verifier checks evidence, entity–attribute binding, multi-hop completeness, and unchanged scope; Format receives only the Verifier artifact | Formatter input omits the original question and copies the supported `Candidate answer` character-for-character into one `<answer>` wrapper. It cannot select, canonicalize, verify, or repair an answer. The structured semantic fields are runtime admission fields, not hidden reasoning supervision. |
