...
| `agent_workflow_env.py::{public_environment_state,_stateful_no_progress_receipt,model_admissible_action_types,action_target_domains}` | Necessary state-conditioned recovery adaptation | FlowSteer action masking at the Canvas edit boundary plus the project MD `preserve -> diagnose -> repair -> augment` recovery policy | A repeated public state--action transition masks blind `CONTINUE`, `DELETE_AGENT` and premature `FINISH`; first expose an in-place free-text contract repair for the preserved environment actor, then admit ordinary Agent/relation augmentation only after persistent measured no-progress. The environment episode and prior public evidence are retained; no WebShop-specific role topology is prescribed. |
| `evaluation_webshop_stepwise_director_v9.yaml`; `model_catalog_webshop_stepwise_v9.yaml` | Isolated evaluation configuration | The fixed v7 ordered 128-task WebShop validation panel, native Average Score/Success Rate evaluator and Direct arm | Advance only the versioned stepwise Director/action-mask condition and isolated artifact paths. Freeze the local Qwen3.5-9B Executor arm because it is the only arm with successful v7 execution receipts; record that condition change explicitly. Training, GRPO, MACE, Bayesian state and Skills remain disabled. |
