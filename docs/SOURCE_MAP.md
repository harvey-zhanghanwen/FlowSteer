...
| `agent_workflow_env.py::{allowed_action_types,model_admissible_action_types,model_admissible_action_targets,step}`; `director.py::{_canvas_observation,action_schema_request}`; `rollout_collector.py::SGLangReceiptDirectorClient._resolve_action_schema`; `train_agentgraph_smoke.py` | FlowSteer action-set reuse plus necessary inference-wire adaptation | FlowSteer `workflow_env.py` BUILDING-state ADD/DELETE/MODIFY/FINISH handling, edit rejection/feedback, explicit terminal action, and the project's configured `agent_graph.actions` | Canvas remains the authority for legal state transitions and returns rejection/execution feedback after every action | The configured scalar or `add_subgraph` profile remains the parser's full legal set. HotpotQA v17 derives a request-scoped sampling schema from the current model-admissible **action-type** subset and publishes exact target domains in the Canvas observation. The unchanged parser and environment still validate every target and mutation after generation; the schema does not choose an action, topology, model, or repair. ADD is available only while capacity remains, target-bearing actions only when their target domain is non-empty, DELETE only after replacement takeover, and FINISH only after the revision-local terminal gate passes. Historical prompt/schema versions remain explicitly addressable rather than being rewritten. |
...
`agentgraph.director.hotpotqa-semantic-recovery.v17` and
`hotpotqa_verified_answer_slot_v1`.  Prompt v16 remains a legacy, explicitly
versioned policy for old receipts; the neutral v10 Director and every other
...
| Current module | Classification | Upstream source | Reused boundary | Minimal adaptation and exclusion |
| --- | --- | --- | --- | --- |
| `director.py::{HOTPOTQA_DIRECTOR_SYSTEM_PROMPT_V17,AgentGraphOrchestrator.action_schema_request,AgentGraphOrchestrator._canvas_observation}`; `agent_workflow_env.py::{model_admissible_action_types,model_admissible_action_targets,finish_admissibility,step}`; `rollout_collector.py::SGLangReceiptDirectorClient._resolve_action_schema` | Direct FlowSteer Canvas reuse plus necessary request-wire adaptation | FlowSteer `workflow_env.py::{InteractiveWorkflowEnv.step,_step_internal,_check_finish_constraints}`, `workflow_builder.py::{TurnRecord,Trajectory,create_action_mask}`, progressive execution feedback, and explicit `FINISH` | State-conditioned Canvas legality, one accepted edit/subgraph followed by one execution, public feedback, revision-local terminal validation, and trajectory/action-token records retain the FlowSteer boundary | v17 keeps a compact task-specific Director instruction and projects the current model-admissible action types into a request-scoped constrained-decoding schema; exact target domains are supplied as observation data. This prevents the model from sampling a currently unavailable action type, but the unchanged parser/Canvas remains authoritative for target IDs, contracts, graph validity, and terminal admission. Beyond the required semantic Reasoner→Verifier→Formatter responsibilities, it fixes no auxiliary topology, action choice, automatic repair, answer, or Skill. FlowSteer's training `action_mask` is not re-described as this inference grammar. |
