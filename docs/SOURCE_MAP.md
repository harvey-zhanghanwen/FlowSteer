...
This section records the source decision made before the multidataset
Tool/ReAct/Coding implementation.  A listed upstream reference is an
implementation source, not an instruction to replace FlowSteer's Director or
progressive Canvas.  Items marked `NOT_IMPLEMENTED` describe the pre-change
state and must not be reported as completed capability.
...
| `NOT_IMPLEMENTED` unified Tool layer; existing partial sources are `src/interactive/qa_retrieval.py::{SkillFlowQARetriever,QARetrievalReceipt,PublicPassageObservation}` and `records.py::ExecutionRecord` | SkillFlow `src/skillev/runtime/tools.py::{ToolRequest,ToolResult,ToolBackend,ToolRegistration,ToolRegistry}`, `runtime/contracts.py::{StructuredAction,ActionKind,BudgetVector}`, `runtime/bounded_agent.py::{_execute_action,_reserve}`; Codex `codex-rs/core/src/tools/router.rs::{ToolCall,ToolRouter::build_tool_call,dispatch_tool_call_with_terminal_outcome}` and `core/src/tools/context.rs::{ToolInvocation,ExecCommandToolOutput,ApplyPatchToolOutput}` | Immutable resource-ID registry, normalized request/result, unknown-resource rejection, action/tool budget settlement, public tool observations, call ID and command outcome receipt semantics | Thin-port the SkillFlow registry. Add `ToolCapability` metadata, asynchronous timeout, and canonical `ToolReceipt` bound to Agent, tool version, arguments, public result, latency, and environment/workspace revision | SkillFlow registration lacks the required dataset/schema/side-effect/version metadata and multi-Agent async receipts; Codex is a Rust product/session stack and is reference-only | All seven datasets; `ToolCapability` and `ToolReceipt` are `NEW_PROJECT_EXTENSION` |
| `src/interactive/qa_retrieval.py::{SkillFlowQARetriever,QARetrievalReceipt,augment_task_with_retrieval}` | SkillFlow `src/skillev/benchmarks/retrieval.py::{RetrievalIndex.open,RetrievalIndex.search,RetrievalIndex.read,QARetrievalEnvironment.execute,QARetrievalEnvironment.validate_completion}` | Read-only versioned index, answer-free `search`/`read` schemas, public passage IDs/text, tool budget, corpus/index identity | Register the retrieval environment as a legal Tool resource; let `allowed_tools` plus `execution_mode=react` control model-driven query/read actions; persist query, top-k, passage IDs, observations, latency, and corpus version | The current wrapper always performs one deterministic prefetch and concatenates results to the question; it cannot express Director tool assignment or a multi-turn query/read loop | HotpotQA and TriviaQA only until another QA protocol explicitly registers a compatible corpus |
| `src/interactive/task_evaluator.py::{_evaluate_environment,_environment_prompt,_parse_environment_action,_environment_actions}` | SkillFlow deployed `src/ragen_adapter.py::{RAGENAdapter.reset,RAGENAdapter.step,ALFWorldEnv.reset,ALFWorldEnv.step,WebShopEnv.reset,WebShopEnv.step}`; SkillFlow `runtime/bounded_agent.py::BoundedAgent.execute_turn` and `runtime/execution.py::{RolloutEnvironmentSession,EnvironmentObservation}` | Real reset, admissible actions, environment step, new public observation, terminal reward/`won`, action parsing, step budget and failure observations | Move execution semantics behind one shared environment execution adapter while retaining the RAGEN implementation and dataset-specific environment configuration; leave only terminal scoring at the evaluator boundary | The current evaluator owns both interaction and scoring and the deployed RAGEN adapter has no AgentGraph or typed receipt boundary; separate ALFWorld/WebShop runtimes would violate the unified Runtime requirement | ALFWorld and WebShop |
| `src/interactive/swebench_adapter.py::OfficialSWEbenchHarness`; Coding Agent runtime is `NOT_IMPLEMENTED` | SkillFlow `training/environment.py::{GenericTaskEnvironment.reset,step,_dispatch_tool,_setup_swe_repo,cleanup,_resolve_repo_path,_handle_list_files,_handle_search_code,_handle_view_file,_handle_str_replace_editor,_generate_workspace_diff,_handle_run_tests,_run_tests_in_swe_env}` and `training/swe_bench_eval.py::evaluate_patch`; Codex `codex-rs/core/src/session/turn.rs::{run_turn,run_sampling_request,drain_in_flight}`, `core/src/tools/handlers/unified_exec/exec_command.rs::ExecCommandHandler`, `core/src/tools/handlers/apply_patch.rs::ApplyPatchHandler`, and `codex-rs/apply-patch/src/invocation.rs::maybe_parse_apply_patch_verified` | Base-commit worktree lifecycle, repository-relative list/search/view, exact edit, workspace diff, targeted test execution, tool result returned to the next model turn, final official harness evaluation | Implement a bounded Python `CodingExecutionAdapter` used by an AgentGraph node with `execution_mode=coding`; reuse the small SkillFlow worktree/editor/test primitives and emit Tool receipts for every command/edit/test | SkillFlow's `GenericTaskEnvironment` is a monolith coupled to a single Supervisor and private state; Codex cannot be embedded without replacing FlowSteer with its Rust session/product stack | SWE-bench Verified; adapter is `NEW_PROJECT_EXTENSION` |
| `src/interactive/agent_runtime.py::UpstreamMessage`; `records.py::{ExecutionRecord,TurnRecord,TrajectoryRecord}`; `rollout_collector.py` | FlowSteer `workflow_builder.py::{TurnRecord,Trajectory,create_action_mask}`; SkillFlow `src/skillev/rollout/artifact.py::{CompletedStepDraft,materialize_trajectory_step,RolloutManifest,RolloutArtifact,finalize_trajectory_record}` and `runtime/execution.py::EnvironmentObservation`; Codex command result types above | Sampled action IDs and masks, graph snapshot/revision, per-call execution receipt, complete reason/action/observation step commit, terminal evaluator receipt | Extend the existing routed message into a canonical typed communication envelope carrying only real artifact type/body, environment/workspace revision, dependency, and Tool receipt references; add execution mode and Tool receipts to persisted execution records | FlowSteer has no tool-level receipts, SkillFlow's trajectory is single-agent sequential, and neither represents typed artifacts across AgentGraph edges | All seven datasets; typed cross-Agent envelope is `NEW_PROJECT_EXTENSION` |
| `src/interactive/skills/{pipeline.py,validator.py,lifecycle.py,retrieval.py,schema.py}` | SkillFlow `src/skillev/evidence/schema.py::{EvidenceRecord,may_update_posterior}`, `evidence/store.py::{EvidenceStore,SplitEvidenceStores}`, `runtime/skill_library.py::{SkillLibrary,SkillLibraryState,active_documents}`, `evolution/retriever.py::{TaskConditionedSkillRetriever.retrieve,applicability_matches,RetrievalDecision}` | Physical evidence split, immutable/versioned ACTIVE library, applicability matching, eligible evidence only, lifecycle and retrieval receipt | Preserve the project's existing paired AgentGraph effect gate; bind conditions/actions to allowed tools, execution mode, environment/workspace protocol, model, policy, tool and evaluator versions; expose only compatible `ACTIVE` records as rejectable Director priors | SkillFlow does not implement the project's same-prefix randomized paired AgentGraph intervention and posterior gate; replacing it would violate the project MD | All seven datasets; existing project algorithm, not a new Skill architecture |
