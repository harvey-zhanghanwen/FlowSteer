...
| `agent_workflow_env.py::{allowed_action_types,model_admissible_action_types,step}`; `director.py::_canvas_observation`; `train_agentgraph_smoke.py` | FlowSteer action-set correction | FlowSteer `workflow_env.py` BUILDING-state ADD/DELETE/MODIFY/FINISH handling and the project's configured `agent_graph.actions` | Canvas remains the authority for legal state transitions and returns rejection/execution feedback after every action | The configured scalar or `add_subgraph` action profile remains the parser's full legal set. The versioned HotpotQA v16 observation additionally projects the current model-admissible subset: ADD only while capacity remains, mutation actions only when their target cardinality exists, DELETE only after replacement takeover, and FINISH only after the revision-local terminal gate passes. The gate exposes both the first measured failure stage and responsible semantic Agent for repair. The historical constrained-decoding condition remains frozen for Round8 reproducibility. |
...
`agentgraph.director.hotpotqa-semantic-recovery.v16` and
...
| Current module | Classification | Upstream source | Reused boundary | Minimal adaptation and exclusion |
| --- | --- | --- | --- | --- |
| `director.py`, `agent_workflow_env.py`; `task_dataset.py::hotpotqa_answer_type_constraint` | FlowSteer reuse plus necessary HotpotQA adaptation | FlowSteer progressive Canvas `edit -> execute -> feedback`, explicit `FINISH`, revision-local execution result, strict action parser, mutation validation, and quotient-graph terminal reachability | One accepted functional subgraph/edit is executed before the next Director action; invalid semantic completion is returned as Canvas feedback. v16 states edge-removal semantics, responsible semantic Agent, and completed replacement takeover | The HotpotQA condition adds one semantic-answer-owning Reasoner, Reasoner–Verifier–Formatter lineage, and `preserve -> diagnose -> repair -> augment`. A disconnected duplicate is deletable only after a current verified Reasoner→Verifier→Formatter lineage owns the semantic artifact and Output identity; this avoids copying obsolete duplicate edges into the active chain. An answer-independent wh-word/auxiliary classifier constrains answer type. It reads no passage, candidate, Ground Truth, or evaluator field. |
