...
| `qa_tool_adapter.py::QARetrievalReactExecutionAdapter`; `react_execution.py::ToolReactExecutionAdapter` | Direct SkillFlow reuse plus bounded factual-retrieval adaptation | SkillFlow `runtime/contracts.py::StructuredAction`, `runtime/bounded_agent.py::execute_turn`, and `benchmarks/retrieval.py` search/read environment | ReAct is the execution schedule `Thought -> Action -> Observation -> Thought`; one admitted structured Tool action produces one public observation and exact Tool receipt | TriviaQA uses one initial search/read opportunity followed, only after evidence rejection, by bounded spelling normalization, alias expansion, entity disambiguation and query rewriting with increasing top-k. Exhaustion records the operational diagnosis `knowledge_base_coverage_failure`; it is not a corpus-level oracle claim and never admits a guessed answer. Recovery r2 propagates the existing typed `tool_plan_exhausted` receipt and preserves successful reads during entity-surface/answer-slot repair. Recovery r7 requires a new successful Reasoner read after provenance rejection. Recovery r10 keeps one compact duplicate-error observation with `repeat_count`; the lossless trajectory remains unchanged. It also gives `evidence_retriever` an answer-free completion schema and validates question scope, concise entity identity, an exact predicate surface, exact evidence span and passage ID against one matching successful read receipt. Recovery r11 keeps receipt/span/alias-lineage failures on evidence recovery but routes field-shape and lexical-field alignment to structured completion repair on that same read. Recovery r12 reuses the existing Reasoner proposition shape as the Retriever's answer-free `evidence_proposition` and validates its object against the question-only `qa_answer_type_constraint`; the Retriever still cannot own `candidate_answer`, `answer_slot`, or `final_answer`. Each execute-on-edit Canvas revision invokes one bounded ReAct call; retaining SkillFlow public Action--Observation history and receipts across a repaired revision is the project adaptation, not a claim that SkillFlow itself supplies cross-Canvas continuation. |
| `task_dataset.py::{qa_question_scope,qa_answer_type_constraint,qa_answer_cardinality_constraint}`; `openai_gateway.py::build_agent_messages`; `agent_workflow_env.py::{_semantic_protocol_issue,_semantic_repair_attribution,finish_admissibility}` | Necessary shared semantic adaptation | Existing answer-free task rendering, SkillFlow Tool receipts, and FlowSteer terminal Format separation | Reasoner consumes public evidence; Verifier gates the candidate; Formatter copies an already verified artifact into the terminal wrapper | The Reasoner owns `candidate_answer`, answer-slot and relation binding.  The Verifier checks entity identity, evidence provenance, question scope and candidate preservation.  A question-only `Who` constraint remains at the `entity` superclass because the answer-bearing grammatical slot may contain a person or organization; evidence and relation binding determine the surface without evaluator input.  A false Verifier verdict over evidence, binding, type/cardinality, scope, multi-hop completeness or candidate surface is attributed back to the Reasoner that owns those fields even when the Runtime wraps the field-level diagnosis, while a malformed Verifier artifact remains a Verifier repair.  A candidate that exactly copies the selected proposition argument and one complete evidence-span entity mention is not invalid merely because a shorter coreferential alias also occurs.  Every proposition must copy its entity surfaces from its own evidence span; a longer question entity is connected to a surname, pronoun or shortened mention through a separate evidence-supported identity proposition.  The Formatter receives the verified artifact rather than solving the question.  Neither accepted answers nor evaluator state enter Director, Agent, query, Tool, or recovery input. |
| `agent_workflow_env.py::{AgentWorkflowEvidenceLineageSnapshot,last_valid_evidence_lineage,recovery_state,_contract_obligation_issue,_recovery_continuation_handoff,_tool_continuation_projection,_reasoner_candidate}`; `agent_runtime.py::AgentRequest`; `rollout_collector.py::{_validate_v3_hierarchical_action_receipt,_last_valid_evidence_lineage_fallback,_execution_record,_runtime_summary}`; `director.py::AgentGraphOrchestrator.run` | FlowSteer/SkillFlow feedback reuse plus project-design-required recovery adaptation | FlowSteer revision-local execution feedback, transactional action rejection, state-conditioned Canvas domains and explicit terminal state; SkillFlow nonterminal public failure observations | Valid evidence, Agent artifacts and communication edges survive later repair attempts; measured failures return to the next Director turn | Recovery follows `preserve -> diagnose -> repair/augment`.  A typed mandatory provider/ReAct repair exposed as the sole live action is accepted before unrelated semantic-spine closure; after execution, normal topology admission resumes. ReAct exhaustion first admits same-Agent repair; a complete repaired call without an increased Tool-receipt count opens non-destructive augmentation. For a unique exhausted Reasoner and one new ReAct Evidence Retriever, r10 introduced a target-phase-bound public continuation handoff. Recovery r11 narrows that handoff to dispatched Tool Action--Observation entries and same-Tool receipts, excludes the source Reasoner's role-specific completion errors, and retains the filtered projection across an initial target-provider failure and model repair. Existing successful and failed dispatch receipts remain in the same bounded Tool budget. Recovery r12 keeps FlowSteer's execute-on-edit boundary while allowing a measured same-role replacement to take over only after its downstream artifact has successfully executed; deletion remains gated until that takeover exists. An internal `CancelledError` caused by sibling fail-fast execution is orchestration cancellation and is not attributed as an Agent failure. The Canvas action mask, live target domain and authoritative admission share one recovery projection: an existing auxiliary ingress is routed first, an exact missing Reasoner -> Verifier -> Formatter edge is closed next, a missing semantic responsibility is added progressively, a prospectively valid Formatter is selected as Output, and only then may one Evidence Retriever/Repair unit augment the exhausted Reasoner. Revision-live successful artifacts retain predecessor identity. Contracts and completion conditions may express semantic responsibilities but cannot inject a concrete candidate, concrete alias mapping or concrete Tool query/top-k/passage ID; neutral Tool references and retrieval responsibilities remain admitted, and exact arguments remain Runtime-owned. Rejected raw sampled actions remain in the lossless trajectory but do not enter the later model-visible accepted transcript. A malformed v3 hierarchical parameter sample retains its exact branch/domain/phase/token/log-probability receipts, is recorded as `action={}` with zero executed prefix, and returns public parse feedback instead of aborting collection; it is never repaired into an action and makes the trajectory GRPO-ineligible. A frozen answer/Runtime/graph tuple is published only after the complete FINISH gate passes. At `max_rounds` the collector may evaluate that last revision-consistent tuple, while retaining `explicit_finish=false`, `termination_reason=max_rounds`, a fallback receipt, and permanent GRPO ineligibility. Without the Env-owned tuple the answer remains null. |
