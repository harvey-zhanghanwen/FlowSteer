...
`agentgraph.director.hotpotqa-semantic-recovery.v15` and
...
| Current module | Classification | Upstream source | Reused boundary | Minimal adaptation and exclusion |
| --- | --- | --- | --- | --- |
| `director.py`, `agent_workflow_env.py`; `task_dataset.py::hotpotqa_answer_type_constraint` | FlowSteer reuse plus necessary HotpotQA adaptation | FlowSteer progressive Canvas `edit -> execute -> feedback`, explicit `FINISH`, revision-local execution result, strict action parser, mutation validation, and quotient-graph terminal reachability | One accepted functional subgraph/edit is executed before the next Director action; invalid semantic completion is returned as Canvas feedback. The existing Canvas action shapes are reused for unconstrained Qwen generation; v15 states edge-removal semantics and identifies the responsible semantic Agent when FINISH is inadmissible | The HotpotQA condition adds one semantic-answer-owning Reasoner, Reasoner–Verifier–Formatter lineage, and `preserve -> diagnose -> repair -> augment`. An answer-independent wh-word/auxiliary classifier constrains answer type: Which-comparisons return an entity rather than the compared metric; who-questions return a person rather than a possessive attribute phrase. It reads no passage, candidate, Ground Truth, or evaluator field. Replacement takeover may transiently retain two same-role nodes, but only one direct Reasoner may satisfy terminal lineage. |
| `qa_tool_adapter.py`, `react_execution.py`, `openai_gateway.py`, `agent_runtime.py`; `task_dataset.py::hotpotqa_question_scope` | Direct SkillFlow execution-mode and structured-output reuse plus typed request adaptation | SkillFlow bounded `StructuredAction` ReAct loop, `runtime/openai_provider.py` `response_format.json_schema`, canonical public Observation continuation, and public `search/read` Action–Observation receipts; FlowSteer state-conditioned legal-action surface; the existing `build_hotpotqa_question` rendering boundary | ReAct remains `Thought -> Action(tool) -> Observation -> Thought -> Final` execution semantics; it is not an Agent role. Under `required_evidence`, state advances through `search(limit=10) -> read₁ -> read₂ -> complete`; entity linking/coreference remain model-generated | HotpotQA v10 constrains the Reasoner completion schema to the question-derived answer type and serializes the answer-bearing proposition first (`proposition_index=0`), followed by bridge propositions. Evidence provenance permits only Unicode/quotation/whitespace canonicalization; semantic paraphrases remain inadmissible. The labelled Verifier wire preserves a numeric candidate as text instead of JSON-scalar coercion. No query, passage, entity, answer, Ground Truth, or evaluator field is injected. |
