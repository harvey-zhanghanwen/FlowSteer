...
| `agent_workflow_env.py::{allowed_action_types,model_admissible_action_types,step}`; `director.py::_canvas_observation`; `train_agentgraph_smoke.py` | FlowSteer action-set correction | FlowSteer `workflow_env.py` BUILDING-state ADD/DELETE/MODIFY/FINISH handling and the project's configured `agent_graph.actions` | Canvas remains the authority for legal state transitions and returns rejection/execution feedback after every action | The configured scalar or `add_subgraph` action profile remains the parser's full legal set. The versioned HotpotQA v14 observation additionally projects the current model-admissible subset: ADD only while capacity remains, mutation actions only when their target cardinality exists, DELETE only after the replacement-takeover gate, and FINISH only after the revision-local terminal gate passes. The gate exposes both the first measured failure stage and responsible semantic Agent for repair. The historical `progressive_add_subgraph_then_finish` constrained-decoding condition remains only for frozen Round8 reproducibility; GRPO token-level action masks remain a separate training receipt. |
...
`agentgraph.director.hotpotqa-semantic-recovery.v14` and
...
| Current module | Classification | Upstream source | Reused boundary | Minimal adaptation and exclusion |
| --- | --- | --- | --- | --- |
| `director.py`, `agent_workflow_env.py` | FlowSteer reuse plus necessary HotpotQA adaptation | FlowSteer progressive Canvas `edit -> execute -> feedback`, explicit `FINISH`, revision-local execution result, strict action parser, mutation validation, and quotient-graph terminal reachability | One accepted functional subgraph/edit is executed before the next Director action; invalid semantic completion is returned as Canvas feedback. The existing Canvas action shapes are reused for unconstrained Qwen generation; v14 states the already-implemented `set_relation(false,false)` edge-removal semantics and identifies the responsible semantic Agent when FINISH is inadmissible | The HotpotQA condition adds one semantic-answer-owning Reasoner, a machine-readable Reasoner–Verifier–Formatter terminal lineage contract, and `preserve -> diagnose -> repair -> augment` recovery state. Additional retrieval nodes use `evidence_retriever` and route into the Reasoner. It does not add a structural reward, fixed answer, Ground Truth field, or benchmark-sample rule. Delete is admitted only for a diagnosed failed or terminal-unreachable node after successful replacement takeover; invalidated correct artifacts and receipts remain retained as previous-revision recovery state but cannot satisfy the current-revision terminal gate. |
| `qa_tool_adapter.py`, `react_execution.py`, `openai_gateway.py`, `agent_runtime.py`; `task_dataset.py::hotpotqa_question_scope` | Direct SkillFlow execution-mode and structured-output reuse plus typed request adaptation | SkillFlow bounded `StructuredAction` ReAct loop, `runtime/openai_provider.py` `response_format.json_schema`, canonical public Observation continuation, and public `search/read` Action–Observation receipts; FlowSteer state-conditioned legal-action surface; the existing `build_hotpotqa_question` rendering boundary | ReAct remains `Thought -> Action(tool) -> Observation -> Thought -> Final` execution semantics; it is not an Agent role. Under `required_evidence`, the public continuation state advances through bounded `search(limit=10) -> read₁ -> read₂ -> complete` transitions. Both reads use distinct unread opaque passage IDs from the same successful ranked result; entity linking and coreference resolution remain the Reasoner's model-generated work | The OpenAI-compatible Executor receives SkillFlow's strict StructuredAction JSON Schema. Search `limit=10`, unread passage IDs, and completion `arguments.value` are request-scoped schema constraints. HotpotQA v9 obtains `question_scope` from the exact terminal `Question:` span, represents `answer_slot` and evidence propositions as typed structures, and binds the candidate to a selected proposition. Evidence provenance accepts only lexical substrings after Unicode/quotation/whitespace canonicalization; semantic paraphrases remain inadmissible. No query, passage, entity, answer, Ground Truth, or evaluator field is injected. |
