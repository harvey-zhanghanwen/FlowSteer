...
Evidence Retriever may receive the unique repair-exhausted Reasoner's public
Action--Observation history and Tool receipts, phase-bound to the new
singleton execution and marked by `continuation_source_agent_id`.  This
temporary projection does not create a graph message edge and carries no
semantic artifact, model transcript, hidden reasoning, Ground Truth or
evaluator state.  Evidence/provenance rejection may reopen an
