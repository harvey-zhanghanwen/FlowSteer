...
| `task_dataset.py::{qa_question_scope,qa_answer_type_constraint,qa_answer_cardinality_constraint}`; `openai_gateway.py::build_agent_messages`; `agent_workflow_env.py::{_role_conditional_semantic_edit_issue_for,_role_conditional_semantic_issue,finish_admissibility}` | Necessary shared semantic adaptation | Existing answer-free task rendering, SkillFlow Tool receipts, and FlowSteer progressive Canvas/terminal validation | Reasoner, Verifier and Format are optional semantic capabilities; ReAct is a per-Agent execution mode; the selected Output may use `output` or `format` | No named `Reasoner -> Verifier -> Formatter` chain is required. Every selected Agent must reach the unique Output, and the routed Output ancestry must contain a successful read receipt. When present, a Reasoner owns candidate binding, a Verifier consumes a routed semantic candidate rather than raw retrieval and preserves it, and a Format Agent is a copy-only sink selected atomically as Output. A generic Output may complete the same receipt-grounded contract without named intermediate roles. Runtime-registered `(execution_mode, allowed_tools)` profiles are exposed as correlated choices so the Director cannot sample an invalid cross-product. Neither accepted answers nor evaluator state enter Director, Agent, query, Tool, or recovery input. |
