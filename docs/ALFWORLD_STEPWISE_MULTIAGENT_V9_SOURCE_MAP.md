# ALFWorld stepwise multi-Agent v9 source map

## 1. Scope

This document records the implementation boundary for
`config/evaluation_alfworld_stepwise_multiagent_v9.yaml`. The condition keeps
the v8 frozen `valid_seen` tasks, model catalog, Tool implementation,
evaluator, action budget, seed and Direct protocol. It registers the v2
minimal-neutral system prompt in the exact transcript receipt boundary after
the failed pre-runtime v8 canary; old v7/v8 artifacts remain immutable.
...
| v9 boundary | Primary source | Classification | Retained or adapted behavior |
|---|---|---|---|
...
## 5. Evaluation invariants retained from v8
...
All v9 artifacts and reports use new versioned paths under
`artifacts/alfworld_stepwise_multiagent_v9/` and
`reports/alfworld_stepwise_multiagent_v9/`. Failed v7/v8 canary results must not be
merged into v9 metrics.

## 6. Acceptance criteria

A v9 Stable Zero or formal evaluation is reportable only if:

1. selected task IDs are the frozen v8 `valid_seen` sequence;
