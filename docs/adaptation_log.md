...
## 2026-09-07 — v2.47收束、v2.48新五题准备

v247完成4/5、一题900秒超时，完整五题原始/长度调整均分N/A；完成子集
原始80%、长度调整77.670785%不能替代固定五题结果。完整报告及逐题原生
结果已提交6c8e75a，v247源码f42e0575已推送原仓库。未启动525或训练。

在可恢复v247父提交上新建 `feature/healthbench-v2.48-evidence-completion-skills-20260907`。
用户最新要求改用另外五题；只按官方文件ID顺序排除现有dev5配置10个唯一ID，
没有检查新题rubric或按得分选择。固定新ID及比较限制见v248协议文件。

仅修已有证据completion采样/校验不一致、引文定位反馈、操作性查询term预算；
候选职责建议增加原问题完整覆盖核对。没有固定Verifier/Reasoner/Formatter，
不关闭ReAct、不调整模型/时限/分母/evaluator、不用草稿充当FINISH。
初次离线配置检查发现候选说明超过既有800字符上限，已压缩通用表达而非放宽
上限，两个配置验证通过。此记录创建时新五题尚未模型调用，协议单测进行中。

冻结前验证完成：证据协议新增29项与既有v241/v242/v247回归共83项通过，
含实际xgrammar CPU编译，无跳过；scope相关49项通过，两个配置验证通过。
...
v251同五题已收束：4/5显式FINISH与有效评分，1题900秒超时。严格全五题
均分N/A；完成四题raw均分0，原生length-adjusted均分0.0444528，不裁负分。
terminal+partial中26个本地Qwen预检token数均与服务端一致，无HTTP400，
无AgentNode类型异常；最大输入19189，未实测触发动态预算收缩，边界仅经
定向测试。远端负载饱和429和ReAct耗尽仍在，不能声明答案质量提升。
完整过程和失败partial保留；离线报告不重新评分。未训练、未启动525。
